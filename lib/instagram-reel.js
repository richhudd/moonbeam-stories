...
async function prepare(req,res,user,body){
  ...
  try{
    ...
    const previewUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(preview.token)}&kind=instagram-reel&v=25066`;
...
async function publish(req,res,user,body){
  ...
  try{
    ...
    const videoUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(token)}&kind=instagram-reel&v=25066&cb=${Date.now()}`;
