...
async function cleanupPreview(shareId){
  if(!shareId)return;await deleteReelVideo(`${shareId}.mp4`);
...
async function prepare(req,res,user,body){
  ...
  try{
    ...
    const rendered=await renderReel(story,user.id,workDir),objectPath=`${preview.share.id}.mp4`;
    await uploadReelVideo(objectPath,rendered.bytes);
    const previewUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(preview.token)}&kind=instagram-reel&v=25063`;
...
async function publish(req,res,user,body){
  ...
  try{
    ...
    const videoUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(token)}&kind=instagram-reel&v=25063&cb=${Date.now()}`;
