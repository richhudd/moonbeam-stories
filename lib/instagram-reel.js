...
async function renderSilentSegment(frame,out,duration,options={}){
  ...
  const skipFadeIn=!!options.skipFadeIn,skipFadeOut=!!options.skipFadeOut;
  const visualFilters=[
  ...
  if(!skipFadeOut)visualFilters.push(`fade=t=out:st=${fadeOut}:d=0.20`);
  visualFilters.push('format=yuv420p');
...
async function renderRevealSegment(fromFrame,toFrame,out,duration=0.60){
  const d=Math.max(0.35,Math.min(1.20,Number(duration)||0.60));
  const transition=Math.min(0.60,d-0.05).toFixed(2);
  const offset=Math.max(0.01,d-Number(transition)).toFixed(2);
  const pre='scale=1120:1992:force_original_aspect_ratio=increase,crop=1120:1992,scale=1080:1920,setsar=1';
  const fc=`[0:v]${pre}[a];[1:v]${pre}[b];[a][b]xfade=transition=smoothleft:duration=${transition}:offset=${offset},format=yuv420p[v]`;
  await runFfmpeg(['-y','-loop','1','-framerate','30','-t',d.toFixed(2),'-i',fromFrame,'-loop','1','-framerate','30','-t',d.toFixed(2),'-i',toFrame,'-filter_complex',fc,'-map','[v]','-an','-r','30','-c:v','libx264','-preset','ultrafast','-crf','24','-profile:v','high','-level','4.1','-movflags','+faststart',out]);
}
async function renderReel(story,userId,workDir,visuals){
  ...
  const coverFrame=path.join(workDir,'frame-2.png');await fs.writeFile(coverFrame,await renderCoverFrame(visuals.cover));frameFiles.push(coverFrame);
  for(let i=0;i<4;i++){const f=path.join(workDir,`frame-${i+3}.png`);await fs.writeFile(f,await renderStoryFrame(pageArt[i],visuals.panels[i]));frameFiles.push(f)}
  const ctaFrame=path.join(workDir,'frame-7.png');await fs.writeFile(ctaFrame,await renderCtaFrame(pageArt[3],visuals.cta));frameFiles.push(ctaFrame);
  await (async()=>{const out=path.join(workDir,'segment-0.mp4');await renderFlashSegment(frameFiles[0],out,0.10);segments.push(out)})();
  await (async()=>{const out=path.join(workDir,'segment-1.mp4');await renderSilentSegment(frameFiles[1],out,4.0,{skipFadeIn:true,skipFadeOut:true});segments.push(out)})();
  await (async()=>{const out=path.join(workDir,'segment-2.mp4');await renderRevealSegment(frameFiles[1],frameFiles[2],out,0.65);segments.push(out)})();
  await (async()=>{const out=path.join(workDir,'segment-3.mp4');await renderSilentSegment(frameFiles[2],out,2.2,{skipFadeIn:true});segments.push(out)})();
  const durations=[2.35,2.35,2.35,2.35,2.8];
  for(let i=3;i<frameFiles.length;i++){
    const out=path.join(workDir,`segment-${i+1}.mp4`);await renderSilentSegment(frameFiles[i],out,durations[i-3]||2.4,{});segments.push(out);
  }
...
async function prepare(req,res,user,body){
  ...
  try{
    ...
    const previewUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(preview.token)}&kind=instagram-reel&v=25077`;
...
async function publish(req,res,user,body){
  ...
  try{
    ...
    const videoUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(token)}&kind=instagram-reel&v=25077&cb=${Date.now()}`;
