...
async function renderReel(story,userId,workDir,visuals){
  ...
  const introFrame=path.join(workDir,'frame-0.jpg');await fs.writeFile(introFrame,visuals.introFrame);frameFiles.push(introFrame);
...
async function prepare(req,res,user,body){
  ...
  try{
    ...
    const previewUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(preview.token)}&kind=instagram-reel&v=25071`;
...
async function publish(req,res,user,body){
  ...
  try{
    ...
    const videoUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(token)}&kind=instagram-reel&v=25071&cb=${Date.now()}`;
