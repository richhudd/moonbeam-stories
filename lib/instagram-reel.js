...
async function waitForInstagramContainer(id,accessToken,label='Instagram Reel'){
  const deadline=Date.now()+150000,delays=[3000,5000,8000,10000,12000];let last='',attempt=0;
...
async function renderSegment(frame,audio,out,duration,options={}){
  ...
  const visualFilters=[
    ...
    "crop=1120:1992",
    "zoompan=z='min(zoom+0.00022,1.035)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=1080x1920:fps=30"
  ];
  if(!skipFadeIn)visualFilters.push("fade=t=in:st=0:d=0.20");
...
async function renderReel(story,userId,workDir,visuals){
  ...
  if(!visuals?.cover||!Array.isArray(visuals?.panels)||visuals.panels.length!==4||!visuals?.cta)throw new Error('Refresh Moonbeam and prepare the Reel again so its text can be captured correctly.');
  ...
  const coverFrame=path.join(workDir,'frame-0.png');await fs.writeFile(coverFrame,await renderCoverFrame(visuals.cover));frameFiles.push(coverFrame);
  for(let i=0;i<4;i++){const f=path.join(workDir,`frame-${i+1}.png`);await fs.writeFile(f,await renderStoryFrame(pageArt[i],visuals.panels[i]));frameFiles.push(f)}
  const ctaFrame=path.join(workDir,'frame-5.png');await fs.writeFile(ctaFrame,await renderCtaFrame(pageArt[3],visuals.cta));frameFiles.push(ctaFrame);
  ...
  for(let i=0;i<frameFiles.length;i++){
    ...
    const out=path.join(workDir,`segment-${i}.mp4`);await renderSegment(frameFiles[i],audioFiles[i],out,duration,{skipFadeIn:i===0});segments.push(out);
  }
...
async function prepare(req,res,user,body){
  ...
  let visuals;try{
    ...
    const cta=await decodeReelJpegDataUrl(body.reelCtaPanelDataUrl,'The Reel ending panel',936,790,260000);visuals={cover,panels,cta};
  }catch(error){return res.status(400).json({ok:false,error:error?.message||'The Reel text could not be captured correctly.'})}
  ...
  try{
    ...
    const previewUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(preview.token)}&kind=instagram-reel&v=25068`;
...
async function publish(req,res,user,body){
  ...
  try{
    ...
    const videoUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(token)}&kind=instagram-reel&v=25068&cb=${Date.now()}`;
