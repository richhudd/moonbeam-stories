...
function introFallback(story){
  const hero=heroCopy(story?.saved_assets||{})||'your child';
  const pair=/and/i.test(hero);
  if(pair)return {heading:`Meet ${hero}`,lines:[`${hero} love stories`,'They love them even more when they’re in them',`Here’s a story Moonbeam made for ${hero}`]};
  return {heading:`Meet ${hero}`,lines:[`${hero} loves stories`,'They love them even more when they’re in them',`Here’s a story Moonbeam made for ${hero}`]};
}
function normaliseIntro(raw,story){
  const fallback=introFallback(story),heading=collapse(raw?.heading||fallback.heading).slice(0,80)||fallback.heading;
  let lines=Array.isArray(raw?.lines)?raw.lines.map(x=>collapse(x)).filter(Boolean).slice(0,3):[];
  while(lines.length<3)lines.push(fallback.lines[lines.length]||'');
  return {heading,lines:lines.slice(0,3)};
}
async function decodeReelSourcePhotoDataUrl(value,label,maxBytes=2000000){
  const match=String(value||'').match(/^data:image\/(?:jpeg|jpg|png|webp);base64,([A-Za-z0-9+/=]+)$/i);if(!match)throw new Error(`${label} was not captured as an image.`);
  const bytes=Buffer.from(match[1],'base64');if(bytes.length<4000||bytes.length>maxBytes)throw new Error(`${label} has an unexpected file size.`);
  const meta=await sharp(bytes).metadata();if(!meta.width||!meta.height)throw new Error(`${label} could not be read.`);
  return sharp(bytes).rotate().jpeg({quality:92}).toBuffer();
}
async function renderIntroFrame(photos,intro){
  const width=1080,height=1920,multi=photos.length>1;
  const bg=await sharp(photos[0]).resize(width,height,{fit:'cover'}).blur(28).modulate({brightness:.52,saturation:.9}).jpeg({quality:88}).toBuffer();
  const photoBoxes=multi?[{x:66,y:260,w:420,h:620},{x:594,y:260,w:420,h:620}]:[{x:190,y:216,w:700,h:860}];
  const photoInputs=[];
  for(let i=0;i<Math.min(photoBoxes.length,photos.length);i++){
    const b=photoBoxes[i];
    const framed=await sharp(photos[i]).resize(b.w,b.h,{fit:'cover'}).extend({top:18,bottom:18,left:18,right:18,background:{r:255,g:250,b:242,alpha:1}}).png().toBuffer();
    photoInputs.push({input:framed,top:b.y,left:b.x});
  }
  const headingY=142,textPanelY=multi?1000:1150,textPanelH=multi?520:430;
  const textLines=intro.lines.map((line,i)=>`<text x="540" y="${textPanelY+112+i*108}" text-anchor="middle" fill="#2e2740" font-family="Georgia, Times New Roman, serif" font-size="${multi?44:48}" font-weight="700">${esc(line)}</text>`).join('');
  const photoCards=photoBoxes.map(b=>`<rect x="${b.x-8}" y="${b.y-8}" width="${b.w+16}" height="${b.h+16}" rx="30" fill="rgba(255,250,242,.28)" stroke="rgba(255,255,255,.55)" stroke-width="2"/>`).join('');
  const overlay=Buffer.from(`<svg width="1080" height="1920" viewBox="0 0 1080 1920" xmlns="http://www.w3.org/2000/svg">
    <rect width="1080" height="1920" fill="rgba(23,19,45,.18)"/>
    <text x="540" y="${headingY}" text-anchor="middle" fill="#fffaf2" font-family="Georgia, Times New Roman, serif" font-size="64" font-weight="700">${esc(intro.heading)}</text>
    ${photoCards}
    <rect x="54" y="${textPanelY}" width="972" height="${textPanelH}" rx="34" fill="rgba(255,250,242,.92)" stroke="#d6be86" stroke-width="3"/>
    ${textLines}
    <text x="540" y="${textPanelY+textPanelH-36}" text-anchor="middle" fill="#72583a" font-family="Arial, Helvetica, sans-serif" font-size="23" font-weight="700" letter-spacing="2">MOONBEAM STORIES</text>
  </svg>`);
  return sharp(bg).composite([...photoInputs,{input:overlay,top:0,left:0}]).png().toBuffer();
}
...
async function renderReel(story,userId,workDir,visuals){
  ...
  if(!visuals?.cover||!Array.isArray(visuals?.panels)||visuals.panels.length!==4||!visuals?.cta||!Array.isArray(visuals?.introPhotos)||!visuals.introPhotos.length)throw new Error('Refresh Moonbeam and prepare the Reel again so its teaser assets can be captured correctly.');
  ...
  const introFrame=path.join(workDir,'frame-0.png');await fs.writeFile(introFrame,await renderIntroFrame(visuals.introPhotos,visuals.intro));frameFiles.push(introFrame);
  const coverFrame=path.join(workDir,'frame-1.png');await fs.writeFile(coverFrame,await renderCoverFrame(visuals.cover));frameFiles.push(coverFrame);
  ...
  const durations=[2.8,2.2,2.35,2.35,2.35,2.35,2.8];
  for(let i=0;i<frameFiles.length;i++){
    const out=path.join(workDir,`segment-${i}.mp4`);await renderSilentSegment(frameFiles[i],out,durations[i]||2.4,{skipFadeIn:i===0});segments.push(out);
  }
  ...
  return {bytes:await fs.readFile(output),intro:visuals.intro};
}
...
async function prepare(req,res,user,body){
  ...
  let visuals;try{
    ...
    if(!Array.isArray(body.reelHeroPhotoDataUrls)||!body.reelHeroPhotoDataUrls.length)throw new Error('The hero photo is missing.');
    const introPhotos=[];for(let i=0;i<Math.min(body.reelHeroPhotoDataUrls.length,2);i++)introPhotos.push(await decodeReelSourcePhotoDataUrl(body.reelHeroPhotoDataUrls[i],`Hero photo ${i+1}`));
    visuals={cover,panels,cta,introPhotos,intro:normaliseIntro(body.reelIntro,story)};
  ...
  try{
    ...
    const previewUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(preview.token)}&kind=instagram-reel&v=25070`;
    ...
    return res.status(200).json({ok:true,token:preview.token,previewUrl,durationStyle:'teaser',intro:rendered.intro});
...
async function publish(req,res,user,body){
  ...
  try{
    ...
    const videoUrl=`${SITE_URL}/api/share?action=asset&token=${encodeURIComponent(token)}&kind=instagram-reel&v=25070&cb=${Date.now()}`;
