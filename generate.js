module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `Write an original bedtime story for a child. Child nickname: ${String(child.name)}. Age: ${Number(child.age)}. Interests: ${child.interests || 'imagination'}. Avoid: ${child.dislikes || 'nothing specific'}. Length: ${child.length || 'medium'}. Tone: ${child.tone || 'cosy and funny'}. Story values: ${(Array.isArray(child.values)?child.values:['Kindness','Curiosity']).join(', ')}. Keep it warm, age-appropriate and non-preachy. No politics, religion, sexual content, graphic violence, horror, dangerous instructions or adult themes. Return JSON only with this shape: {"title":"string","opening":"string","pages":[{"text":"string","illustration_prompt":"string"},{"text":"string","illustration_prompt":"string"},{"text":"string","illustration_prompt":"string"},{"text":"string","illustration_prompt":"string"}],"closing":"string"}`;
    const r = await fetch('https://api.openai.com/v1/responses', {
      method:'POST',
      headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},
      body:JSON.stringify({model:'gpt-5.6-luna', input:prompt, max_output_tokens:2500})
    });
    ...
    let story;
    try { story=JSON.parse(output); } catch (e) {
