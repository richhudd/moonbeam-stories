...
function coverHeroNames(book=currentBook){const names=(book?.child?.cast||[]).filter(m=>m?.role==='hero').map(m=>String(m.name||'').trim()).filter(Boolean).slice(0,2);if(!names.length&&book?.child?.name)names.push(String(book.child.name).trim());return [...new Set(names)]}
