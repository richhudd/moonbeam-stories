...
function makeStoryCacheId(s,child){return stableHash(JSON.stringify({t:s.title||'',o:s.opening||'',b:s.character_bible||'',vs:s.visual_style_id||s.pages?.[0]?.visual_style_id||'classic_moonbeam',p:(s.pages||[]).map(x=>[x.text||'',x.illustration_prompt||'']),c:s.closing||'',n:child?.name||'',a:child?.age||'',r:child?.referencePhoto?stableHash(child.referencePhoto.slice(-1200)):'none'}))}
...
function getCoverPrompt(book){
 ...
 return `Front cover illustration for an original premium children's adventure called “${book.title}”. Main child/hero: ${book.child?.name||'the child'}, age ${book.child?.age||7}. Interests: ${book.child?.interests||'imaginative adventures'}. Story world and character continuity: ${book.character_bible||'Keep the hero appealing and visually consistent with the interior illustrations.'} Opening: ${book.opening||''} Visual clue from the first scene: ${first}. Compose this specifically as a striking real children's BOOK COVER: one clear focal character, a strong sense of mystery or adventure, compelling depth, lighting appropriate to the story's actual setting and time of day, premium publishing-quality visual storytelling, expressive but reassuring. Keep the central and upper areas sufficiently calm and uncluttered for title typography that will be overlaid by the app. Absolutely no words, letters, captions, logos, signs or readable text anywhere in the image. Follow the story's actual setting and time of day; do not default to nighttime, moonlight, stars, darkness, sleep or bedtime imagery unless the story itself specifically calls for them.`
}
