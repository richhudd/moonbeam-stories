...
async function applyAuthSession(session){
 ...
 if(currentUser){
   ...
   await Promise.all([loadCloudStories(),loadStoryCredits()]);
...
async function generateStory(){
 ...
 try{
   ...
   if(preparingCopy)preparingCopy.textContent='Moonbeam is illustrating a private first story…';
   const storyboardArtwork=await createStoryboardArtwork(data.plan,child,child.generationRunId);
   if(preparingCopy)preparingCopy.textContent='Moonbeam is finding a better story in the finished pictures…';
   ...
   const finalWriterChild={name:child.name,age:child.age,gender:child.gender||null,language:child.language,languageName:child.languageName,cast:Array.isArray(child.cast)?child.cast.map(m=>({name:m.name,kind:m.kind,role:m.role,age:m.age||null,gender:m.gender||null,animal_type:m.animal_type||null,breed:m.breed||null})):[]};
   try{finalResponse=await fetch('/api/generate',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${finalToken}`},body:JSON.stringify({action:'finalize-storyboard-story',child:finalWriterChild,images:thumbnails,generationRunId:child.generationRunId})});finalRaw=await finalResponse.text()}catch(networkError){throw new Error(developerGenerationDiagnostic('final story request',networkError,finalResponse,finalRaw))}
   ...
   // V251.62: restore visual-production metadata only after Story B has finished.
   // Story B receives only the six finished images plus mechanical Cast/age/language data: no Story A, parent idea, storyboard, character bible or visual prompts.
   ...
   clearSetupDraft();
   renderStory(finalData.story,null,child,{prebuiltArtwork:storyboardArtwork,developerStoryA:instagramDeveloperAccess?data.developer_story_a:null})
 }catch(e){
...
function renderStory(s,image,child,options={}){
 ...
 const compareStories=$('developerCompareStories');if(compareStories)compareStories.onclick=showDeveloperStoryComparison;
...
function showDeveloperStoryComparison(){
 if(!instagramDeveloperAccess||!currentBook?.developerStoryA||currentBook.isShared)return;
 let d=$('developerStoryComparison');if(d)d.remove();
 const a=currentBook.developerStoryA||{},aSpreads=Array.isArray(a.spreads)?a.spreads:[];
 const bSpreads=[currentBook.opening,...(currentBook.pages||[]).map(p=>p?.text||''),currentBook.closing];
 const render=(title,spreads)=>`<section style="min-width:0;flex:1 1 340px"><h3 style="margin:.2rem 0 .8rem">${escapeHtml(title)}</h3>${spreads.map((x,i)=>`<div style="margin:0 0 1rem;padding:0 0 1rem;border-bottom:1px solid rgba(0,0,0,.12)"><strong>Page ${i+1}</strong><div style="margin-top:.35rem;white-space:pre-wrap;line-height:1.55">${escapeHtml(String(x||''))}</div></div>`).join('')}</section>`;
 d=document.createElement('div');d.id='developerStoryComparison';d.style.cssText='position:fixed;inset:0;z-index:100000;background:rgba(0,0,0,.62);padding:24px;overflow:auto;';
 d.innerHTML=`<div style="max-width:1180px;margin:0 auto;background:#fff;color:#211b26;border-radius:16px;padding:20px;box-shadow:0 16px 60px rgba(0,0,0,.35)"><div style="display:flex;justify-content:space-between;gap:16px;align-items:center;margin-bottom:12px"><div><strong>Developer story comparison</strong><div style="font-size:13px;opacity:.7">Story A created the pictures. Story B saw only the finished pictures.</div></div><button id="developerStoryComparisonClose" class="secondary" type="button">Close</button></div><div style="display:flex;flex-wrap:wrap;gap:28px;align-items:flex-start">${render(`Story A — ${a.title||'private seed story'}`,aSpreads)}${render(`Story B — ${currentBook.title||'final story'}`,bSpreads)}</div></div>`;
 document.body.appendChild(d);$('developerStoryComparisonClose').onclick=()=>d.remove();d.addEventListener('click',e=>{if(e.target===d)d.remove()});
}
...
function kindleRenderTextPageImage(text,label=''){const W=1080,H=1350,c=document.createElement('canvas');c.width=W;c.height=H;const ctx=c.getContext('2d',{alpha:false});ctx.fillStyle='#f7f2e8';ctx.fillRect(0,0,W,H);ctx.fillStyle='#17132d';ctx.textAlign='left';ctx.textBaseline='top';const clean=String(text||'').trim(),words=clean.split(/\s+/).filter(Boolean).length;let size=words>220?35:words>175?39:words>130?43:47;const maxW=850,lineH=Math.round(size*1.48);if(label){ctx.fillStyle='#9a8350';ctx.font='700 24px Georgia, serif';ctx.textAlign='center';ctx.fillText(String(label).toUpperCase(),W/2,92);ctx.textAlign='left';ctx.fillStyle='#17132d'}ctx.font=`${size}px Georgia, serif`;const paragraphs=clean.split(/\n{2,}/).filter(Boolean),blocks=paragraphs.map(p=>kindleWrapLines(ctx,p,maxW));const totalLines=blocks.reduce((n,b)=>n+b.length,0),paraGaps=Math.max(0,blocks.length-1)*Math.round(lineH*.5),contentH=totalLines*lineH+paraGaps;let y=Math.max(label?165:100,(H-contentH)/2);for(const lines of blocks){for(const line of lines){ctx.fillText(line,115,y);y+=lineH}y+=Math.round(lineH*.5)}ctx.fillStyle='#cfb069';ctx.textAlign='center';ctx.font='38px Georgia, serif';ctx.fillText('☾',W/2,1265);return kindleCanvasData(c)}
