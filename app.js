...
function renderStory(s,image,child){status.textContent='';const el=$('story');el.classList.remove('hidden');el.innerHTML=(image?`<img class="hero" src="${image}" alt="Story illustration">`:'')+`<h1>${escapeHtml(s.title)}</h1><p class="page">${escapeHtml(s.opening||'')}</p>`+(s.pages||[]).map(p=>`<article class="page">${escapeHtml(p.text||'')}</article>`).join('')+`<p class="closing">${escapeHtml(s.closing||'')}</p><div class="actions"><button class="secondary" id="save" type="button">♡ Save story</button><button class="secondary" id="newStory" type="button">↟ New story</button></div>`;$('save').onclick=()=>{saved.unshift({title:s.title,story:s,image,child,at:new Date().toISOString()});saved=saved.slice(0,12);localStorage.setItem('moonbeamStories',JSON.stringify(saved));renderLibrary();$('save').textContent='♥ Saved'};$('newStory').onclick=()=>window.scrollTo({top:0,behavior:'smooth'});el.scrollIntoView({behavior:'smooth'})}
