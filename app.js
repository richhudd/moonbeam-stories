...
async function loadStoryCredits(){
 ...
 if(!error && Number(data?.lifetime_granted||0)===0){
   ...
   if(!error && Number(data?.balance||0)===0 && claim?.reason==='device_used'){
     const el=$('creditStatus');if(el)el.innerHTML='<strong>This device has already enjoyed its free Moonbeam story.</strong> Your saved stories are still free to read; add story credits to create new adventures.';
...
function applyShareRecipientLocale(row){const sx=shareT(shareStoryLanguage());const n=row.querySelector('.share-recipient-name'),e=row.querySelector('.share-recipient-email'),b=row.querySelector('.share-remove');if(n)n.placeholder=sx.recipientName;if(e)e.placeholder=sx.email;if(b)b.setAttribute('aria-label',sx.remove)}
function addShareRecipient(name='',email=''){const box=$('shareRecipients');if(!box||box.children.length>=10)return;const row=document.createElement('div');row.className='share-recipient';row.innerHTML=`<input class="share-recipient-name" maxlength="80" value="${escapeHtml(name)}"><input class="share-recipient-email" type="email" value="${escapeHtml(email)}"><button type="button" class="share-remove">×</button>`;row.querySelector('.share-remove').onclick=()=>{if(box.children.length>1)row.remove()};box.appendChild(row);applyShareRecipientLocale(row)}
