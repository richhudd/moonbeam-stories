...
async function kindleLoadImage(src){return new Promise(async(resolve,reject)=>{try{const img=new Image();img.onload=()=>resolve(img);img.onerror=()=>reject(new Error('A Kindle page image could not be prepared.'));img.src=src instanceof Blob?URL.createObjectURL(src):src;if(img.decode)await img.decode().catch(()=>{})}catch(e){reject(e)}})}
...
async function downloadKindleEbook(){
 ...
 try{
  ...
  if(book.currentPage===book.pages.length+2)renderBookPage(book.currentPage);
  const coverData=await captureReaderCoverForInstagram();
  const coverBytes=await kindleBytesFromSource(coverData);
  ...
  const authorPhoto=await kindleHeroPhoto(book,profile);if(!authorPhoto)throw new Error(`${profile.name}'s Moonbeam profile photo is required for the author page.`);const authorBytes=await kindleBytesFromSource(authorPhoto);
  ...
  files.push({name:'OEBPS/styles.css',data:css},{name:'OEBPS/images/cover.jpg',data:coverBytes});
  illustrations.forEach((b,i)=>files.push({name:`OEBPS/images/illustration-${i+1}.webp`,data:b}));
  files.push({name:'OEBPS/cover.xhtml',data:kindleXhtml(book.title,`<main class="full-page cover-page"><img src="images/cover.jpg" alt="${kindleXml(book.title)}"/></main>`)});
  const spine=['cover'],manifest=[`<item id="cover" href="cover.xhtml" media-type="application/xhtml+xml"/>`,`<item id="css" href="styles.css" media-type="text/css"/>`,`<item id="coverimg" href="images/cover.jpg" media-type="image/jpeg" properties="cover-image"/>`];
  ...
  files.push({name:'OEBPS/nav.xhtml',data:`<?xml version="1.0" encoding="utf-8"?><html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops"><head><title>Contents</title></head><body><nav epub:type="toc"><ol><li><a href="cover.xhtml">${kindleXml(book.title)}</a></li><li><a href="text1.xhtml">Story</a></li><li><a href="author.xhtml">About the Author</a></li></ol></nav></body></html>`});
