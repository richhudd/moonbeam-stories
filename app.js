...
function coverSubtitle(book=currentBook){return developerChildCoverCredit(book)||coverT().forChild(coverHeroList(coverHeroNames(book),language))}
...
async function runDeveloperCorrection(kind){
 ...
 try{
  if(target==='cover'&&(kind==='title'||kind==='byline')){
   ...
   persistCurrentDraft();closeDeveloperCorrection();renderBookPage(0);return;
  }
  if(target==='cover'){
