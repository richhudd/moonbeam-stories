...
'en-GB':{signIn:'Sign in',kicker:'PERSONALISED BEDTIME MAGIC',headline:'Their world. Their adventure. <em>Their bedtime story.</em>',copy:'Create beautiful illustrated stories inspired by your child, the things they love and the values that matter to you.',start:'Create their first story →',free:'3 stories free · No card required',demo:'Moonbeam personalised story demonstration',demoAlt:"A child's photo transformed into personalised Moonbeam story illustrations"},
'en-US':{signIn:'Sign in',kicker:'PERSONALIZED BEDTIME MAGIC',headline:'Their world. Their adventure. <em>Their bedtime story.</em>',copy:'Create beautiful illustrated stories inspired by your child, the things they love and the values that matter to you.',start:'Create their first story →',free:'3 stories free · No card required',demo:'Moonbeam personalized story demonstration',demoAlt:"A child's photo transformed into personalized Moonbeam story illustrations"},
'es-ES':{signIn:'Iniciar sesión',kicker:'MAGIA PERSONALIZADA PARA LA HORA DE DORMIR',headline:'Su mundo. Su aventura. <em>Su cuento para dormir.</em>',copy:'Crea preciosos cuentos ilustrados inspirados en tu hijo, en lo que le gusta y en los valores que son importantes para ti.',start:'Crear su primer cuento →',free:'3 cuentos gratis · Sin tarjeta',demo:'Demostración de un cuento personalizado de Moonbeam',demoAlt:'La foto de un niño transformada en ilustraciones personalizadas de Moonbeam'},
'es-419':{signIn:'Iniciar sesión',kicker:'MAGIA PERSONALIZADA PARA DORMIR',headline:'Su mundo. Su aventura. <em>Su cuento para dormir.</em>',copy:'Crea hermosos cuentos ilustrados inspirados en tu hijo, en lo que le gusta y en los valores que son importantes para ti.',start:'Crear su primer cuento →',free:'3 cuentos gratis · Sin tarjeta',demo:'Demostración de un cuento personalizado de Moonbeam',demoAlt:'La foto de un niño transformada en ilustraciones personalizadas de Moonbeam'},
'fr-FR':{signIn:'Se connecter',kicker:'UNE MAGIE PERSONNALISÉE POUR LE COUCHER',headline:'Son monde. Son aventure. <em>Son histoire du soir.</em>',copy:'Créez de magnifiques histoires illustrées inspirées de votre enfant, de ce qu’il aime et des valeurs qui comptent pour vous.',start:'Créer sa première histoire →',free:'3 histoires gratuites · Sans carte bancaire',demo:'Démonstration d’une histoire personnalisée Moonbeam',demoAlt:'La photo d’un enfant transformée en illustrations personnalisées Moonbeam'},
'de-DE':{signIn:'Anmelden',kicker:'PERSONALISIERTE MAGIE ZUR SCHLAFENSZEIT',headline:'Seine Welt. Sein Abenteuer. <em>Seine Gute-Nacht-Geschichte.</em>',copy:'Erstelle wunderschön illustrierte Geschichten, inspiriert von deinem Kind, seinen Interessen und den Werten, die dir wichtig sind.',start:'Erste Geschichte erstellen →',free:'3 Geschichten kostenlos · Keine Karte nötig',demo:'Beispiel einer personalisierten Moonbeam-Geschichte',demoAlt:'Das Foto eines Kindes wird zu personalisierten Moonbeam-Illustrationen'},
'it-IT':{signIn:'Accedi',kicker:'MAGIA PERSONALIZZATA DELLA BUONANOTTE',headline:'Il suo mondo. La sua avventura. <em>La sua storia della buonanotte.</em>',copy:'Crea splendide storie illustrate ispirate a tuo figlio, a ciò che ama e ai valori importanti per te.',start:'Crea la sua prima storia →',free:'3 storie gratis · Nessuna carta richiesta',demo:'Dimostrazione di una storia personalizzata Moonbeam',demoAlt:'La foto di un bambino trasformata in illustrazioni personalizzate Moonbeam'},
'pt-BR':{signIn:'Entrar',kicker:'MAGIA PERSONALIZADA NA HORA DE DORMIR',headline:'O mundo da criança. A aventura dela. <em>A história de dormir dela.</em>',copy:'Crie lindas histórias ilustradas inspiradas na criança, no que ela ama e nos valores importantes para você.',start:'Criar a primeira história →',free:'3 histórias grátis · Sem cartão',demo:'Demonstração de uma história personalizada Moonbeam',demoAlt:'A foto de uma criança transformada em ilustrações personalizadas Moonbeam'},
'pl-PL':{signIn:'Zaloguj się',kicker:'SPERSONALIZOWANA MAGIA NA DOBRANOC',headline:'Jego świat. Jego przygoda. <em>Jego opowieść na dobranoc.</em>',copy:'Twórz piękne ilustrowane historie inspirowane Twoim dzieckiem, tym, co kocha, oraz wartościami, które są dla Ciebie ważne.',start:'Stwórz pierwszą historię →',free:'3 historie gratis · Bez karty',demo:'Prezentacja spersonalizowanej historii Moonbeam',demoAlt:'Zdjęcie dziecka przekształcone w spersonalizowane ilustracje Moonbeam'}
};
...
function beginStory(mode='self'){if(!currentBook)return;currentBook.readingMode=mode;currentBook.mobileSide='text';const cover=$('coverView'),book=$('book'),controls=$('bookControls');if(cover){cover.classList.add('hidden');cover.hidden=true;cover.style.setProperty('display','none','important')}if(book){book.classList.remove('hidden');book.hidden=false;book.style.removeProperty('display')}if(controls){controls.classList.remove('hidden');controls.hidden=false}renderBookPage(0);if(mode==='narrated')scheduleNarration(120)}
...
async function savedAssetUrl(path){if(!path||!currentBook?.isSaved)return null;if(currentBook.savedAssetUrls[path])return currentBook.savedAssetUrls[path];const {data,error}=await supabaseClient.storage.from('saved-story-art').download(path);if(error)throw error;const url=URL.createObjectURL(data);currentBook.savedAssetUrls[path]=url;return url}
...
function savedStoryCopy(){return t().savedTranslation||{readIn:'Read in',open:'Open',translate:'Translate',translating:'Translating…',free:'Translation does not use a story credit.',readOnly:'Saved stories reopen in read-it-myself mode.',signIn:'Sign in to translate saved stories.',failed:'Translation could not be created.'}}
...
function renderLibrary(){
 ...
 if(!items.length){l.innerHTML=`<p class="muted">${escapeHtml(t().noSaved)}</p>${currentUser?'':`<p class="cloud-note">${escapeHtml(c.signIn)}</p>`}`;return}
 const opts=x=>Object.entries(languageNames).map(([code,name])=>`<option value="${code}"${code===x.language?' selected':''}>${escapeHtml(name)}${x.translations?.[code]?' ✓':''}</option>`).join('');
 if(currentUser){l.innerHTML=items.map((x,i)=>`<div class="library-item library-item-v67"><button class="library-open" type="button" onclick="openSaved(${i})">📖 ${escapeHtml(x.title)} <small>— ${escapeHtml(x.child?.name||'')}</small></button><div class="saved-language"><label>${escapeHtml(c.readIn)} <select id="savedLang${i}" onchange="updateSavedAction(${i})">${opts(x)}</select></label><button class="secondary saved-translate" type="button" onclick="openOrTranslateSaved(${i})">🌐 ${escapeHtml(c.open)}</button></div><small class="saved-free-note">${escapeHtml(c.free)} ${escapeHtml(c.readOnly)}</small><button class="library-delete" type="button" onclick="deleteSavedStory('${escapeHtml(x.id)}')">${escapeHtml(t().delete)}</button></div>`).join('')}else{l.innerHTML=items.map((x,i)=>`<div class="library-item"><button type="button" onclick="openSaved(${i})">📖 ${escapeHtml(x.title)} <small>— ${escapeHtml(x.child?.name||'')}</small></button><small>${escapeHtml(c.signIn)}</small></div>`).join('')}
}
async function openSavedVersion(x,target){const story=savedStoryForLanguage(x,target)||x.story;const displayLanguage=savedStoryForLanguage(x,target)?target:x.language;if(displayLanguage&&locales[displayLanguage]){language=displayLanguage;$('language').value=language;localStorage.setItem('moonbeamLanguage',language);applyLocale()}const child={...(x.child||{}),language:displayLanguage};delete child.referencePhoto;child.generationRunId=null;renderStory(story,null,child,{isSaved:true,visualCacheId:`saved:${x.id}`,savedAssets:x.savedAssets||{}});}
window.openSaved=async i=>{const items=currentUser?cloudStories:saved,x=items[i];if(x)await openSavedVersion(x,x.language)};
window.updateSavedAction=i=>{const x=cloudStories[i],sel=$(`savedLang${i}`);if(!x||!sel)return;const c=savedStoryCopy(),button=sel.closest('.saved-language')?.querySelector('button');if(button)button.textContent='🌐 '+((sel.value===x.language||x.translations?.[sel.value])?c.open:c.translate)};
window.openOrTranslateSaved=async i=>{const x=cloudStories[i];if(!x)return;const target=$(`savedLang${i}`)?.value||x.language,c=savedStoryCopy(),button=document.querySelector(`#savedLang${i}`)?.closest('.saved-language')?.querySelector('button');if(target===x.language||x.translations?.[target]){await openSavedVersion(x,target);return}if(button){button.disabled=true;button.textContent='🌐 '+c.translating}try{const token=await currentAccessToken();if(!token)throw new Error(c.signIn);const r=await fetch('/api/translate-story',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`},body:JSON.stringify({storyId:x.id,targetLanguage:target})});const data=await r.json().catch(()=>({}));if(!r.ok||!data.translation)throw new Error(data.error||c.failed);x.translations=x.translations||{};x.translations[target]=data.translation;renderLibrary();await openSavedVersion(x,target)}catch(e){alert(e.message||c.failed);if(button){button.disabled=false;button.textContent='🌐 '+c.translate}}};
