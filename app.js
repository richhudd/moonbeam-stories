...
async function savedAssetUrl(path){if(!path||!currentBook?.isSaved)return null;if(currentBook.savedAssetUrls[path])return currentBook.savedAssetUrls[path];if(currentBook.isShared){const kind=String(path).replace(/^share:/,'');const r=await fetch(`/api/shared-asset?token=${encodeURIComponent(currentBook.shareToken)}&kind=${encodeURIComponent(kind)}`);if(!r.ok)throw new Error('Shared illustration unavailable.');const url=URL.createObjectURL(await r.blob());currentBook.savedAssetUrls[path]=url;return url}const {data,error}=await supabaseClient.storage.from('saved-story-art').download(path);if(error)throw error;const url=URL.createObjectURL(data);currentBook.savedAssetUrls[path]=url;return url}
...
async function loadSharedStory(token){
 try{
  const r=await fetch(`/api/shared-story?token=${encodeURIComponent(token)}`,{cache:'no-store'}),data=await r.json();if(!r.ok)throw new Error(data?.error||'This shared story is unavailable.');
