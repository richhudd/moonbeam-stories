...
function applyLocale(){
 ...
 renderValues(); renderLibrary(); applyInterfaceLocale();
}
...
function exitStoryToSetup(){stopNarration();document.body.classList.remove('story-mode');$('story')?.classList.add('hidden');goSetupPage(5)}
function animateBookTurn(direction,action){
 ...
 let changed=false;const change=()=>{if(changed)return;changed=true;action()};setTimeout(change,360);
 sheet.addEventListener('animationend',()=>{change();sheet.remove();book.classList.remove('turning')},{once:true});
 setTimeout(()=>{if(sheet.isConnected){change();sheet.remove();book.classList.remove('turning')}},850)
}
...
async function openSavedVersion(x,target){
 const story=savedStoryForLanguage(x,target)||x.story;
 const displayLanguage=savedStoryForLanguage(x,target)?target:x.language;
 if(displayLanguage&&locales[displayLanguage]){language=displayLanguage;$('language').value=language;localStorage.setItem('moonbeamLanguage',language);applyLocale()}
 const child={...(x.child||{}),language:displayLanguage};child.generationRunId=x.generationRunId||child.generationRunId||null;if(!child.referencePhoto){child.referencePhoto=await childPhotoGet(currentPhotoKey(child.profileId||null))}
 const visualCacheId=makeStoryCacheId(x.story,child);
 renderStory(story,x.image||null,child,{isSaved:true,visualCacheId});
}
