...
async function recoverMissingSavedStoryArt(x,book){
 ...
 try{
  const total=book.pages.length+2,images=[];
  for(let i=0;i<total;i++){
   const prompt=getIllustrationPrompt(i),suffix=`:${i}:${stableHash(String(prompt||''))}`;
   const image=await persistentImageFindBySuffix(suffix);
   if(!image)return false;
   images.push(image);
  }
  const base=`${currentUser.id}/${x.id}`,assets={version:2,cover:`${base}/cover.webp`,pages:[]};
  let r=await supabaseClient.storage.from('saved-story-art').upload(assets.cover,dataUrlToBlob(images[0]),{contentType:'image/webp',upsert:true,cacheControl:'31536000'});if(r.error)throw r.error;
  for(let i=0;i<total;i++){const path=`${base}/page-${i}.webp`;r=await supabaseClient.storage.from('saved-story-art').upload(path,dataUrlToBlob(images[i]),{contentType:'image/webp',upsert:true,cacheControl:'31536000'});if(r.error)throw r.error;assets.pages.push(path)}
  const u=await supabaseClient.from('saved_stories').update({saved_assets:assets}).eq('id',x.id).select('saved_assets').single();if(u.error)throw u.error;
  x.savedAssets=u.data.saved_assets;book.savedAssets=x.savedAssets;return true;
 }catch(e){console.error('saved artwork recovery failed',e);return false}
}
async function openSavedVersion(x,target){const story=savedStoryForLanguage(x,target)||x.story;const displayLanguage=savedStoryForLanguage(x,target)?target:x.language;if(displayLanguage&&locales[displayLanguage]){language=displayLanguage;$('language').value=language;localStorage.setItem('moonbeamLanguage',language);applyLocale()}const child={...(x.child||{}),language:displayLanguage};delete child.referencePhoto;child.generationRunId=null;renderStory(story,null,child,{isSaved:true,visualCacheId:`saved:${x.id}`,savedAssets:x.savedAssets||{}});if(!x.savedAssets?.pages?.length)await recoverMissingSavedStoryArt(x,currentBook);}
