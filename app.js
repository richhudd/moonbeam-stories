...
function cloudChildPhotoPath(profileId){return currentUser&&profileId?`${currentUser.id}/${profileId}/reference.jpg`:null}
...
async function downloadCloudChildPhoto(profileId){const path=cloudChildPhotoPath(profileId);if(!path||!supabaseClient)return null;try{const {data,error}=await supabaseClient.storage.from(CHILD_PHOTO_BUCKET).download(path);if(error||!data)return null;return await blobToDataUrl(data)}catch{return null}}
async function uploadCloudChildPhoto(profileId,image){const path=cloudChildPhotoPath(profileId);if(!path||!image||!supabaseClient)return false;try{const {error}=await supabaseClient.storage.from(CHILD_PHOTO_BUCKET).upload(path,dataUrlToBlob(image),{contentType:'image/jpeg',upsert:true,cacheControl:'3600'});if(error){console.warn('Child photo cloud upload failed:',error.message);return false}cloudPhotoResolved.set(profileId,image);return true}catch(e){console.warn('Child photo cloud upload failed:',e?.message||e);return false}}
async function deleteCloudChildPhoto(profileId){const path=cloudChildPhotoPath(profileId);if(!path||!supabaseClient)return;try{const {error}=await supabaseClient.storage.from(CHILD_PHOTO_BUCKET).remove([path]);if(error)console.warn('Child photo cloud removal failed:',error.message)}catch{}cloudPhotoResolved.delete(profileId)}
async function localChildPhotoForProfile(profileId){const exactKey=currentPhotoKey(profileId),exact=await childPhotoGet(exactKey);if(exact)return exact;const db=await openImageDb();if(!db||!profileId)return null;const suffix=`:${profileId}`;const found=await new Promise(resolve=>{try{const tx=db.transaction(CHILD_PHOTO_STORE,'readonly'),req=tx.objectStore(CHILD_PHOTO_STORE).getAll();req.onsuccess=()=>{const rows=(req.result||[]).filter(r=>String(r.key||'').endsWith(suffix)&&r.image).sort((a,b)=>(b.at||0)-(a.at||0));resolve(rows[0]?.image||null)};req.onerror=()=>resolve(null)}catch{resolve(null)}});if(found)await childPhotoPut(exactKey,found);return found}
async function childPhotoGetForProfile(profileId){
 if(!profileId)return childPhotoGet(currentPhotoKey());
 if(cloudPhotoResolved.has(profileId))return cloudPhotoResolved.get(profileId);
 const local=await localChildPhotoForProfile(profileId);
 if(!currentUser){if(local)cloudPhotoResolved.set(profileId,local);return local}
 // Cloud is authoritative across devices. Existing V169 local-only photos are migrated
 // automatically only when no cloud copy exists yet.
 const cloud=await downloadCloudChildPhoto(profileId);
 if(cloud){await childPhotoPut(currentPhotoKey(profileId),cloud);cloudPhotoResolved.set(profileId,cloud);return cloud}
 if(local){await uploadCloudChildPhoto(profileId,local);cloudPhotoResolved.set(profileId,local);return local}
 cloudPhotoResolved.set(profileId,null);return null
}
