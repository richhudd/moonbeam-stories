...
async function loadCastMembers246(){
 ...
 const {data,error}=await supabaseClient.from('cast_members').select('id,kind,name,age,relationship,animal_type,created_at').order('created_at',{ascending:true});
...
async function renderCast246(){
 ...
 await Promise.all([render(children,castMembers246.filter(m=>m.kind==='child'),x.emptyChild),render(adults,castMembers246.filter(m=>m.kind==='adult'),x.emptyAdult),render(pets,castMembers246.filter(m=>m.kind==='pet'),x.emptyPet)]);
}
...
async function saveCastEditor246(){
 ...
 if(!name){status.textContent=x.required;return}if(kind==='child'&&(age<3||age>12)){status.textContent=x.ageError;return}if(kind==='adult'&&!relationship){status.textContent=x.relationshipRequired;return}if(kind==='pet'&&!animal){status.textContent=x.animalRequired;return}
