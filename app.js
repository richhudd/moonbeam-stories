...
async function savedAssetUrl(path){if(!path||!currentBook?.isSaved)return null;if(currentBook.savedAssetUrls[path])return currentBook.savedAssetUrls[path];if(currentBook.isShared){const kind=String(path).replace(/^share:/,'');const r=await fetch(`/api/share?action=asset&token=${encodeURIComponent(currentBook.shareToken)}&kind=${encodeURIComponent(kind)}`);if(!r.ok)throw new Error('Shared illustration unavailable.');const url=URL.createObjectURL(await r.blob());currentBook.savedAssetUrls[path]=url;return url}const {data,error}=await supabaseClient.storage.from('saved-story-art').download(path);if(error)throw error;const url=URL.createObjectURL(data);currentBook.savedAssetUrls[path]=url;return url}
...
function ensureShareDialog(){
 let d=$('shareStoryDialog');if(d)return d;d=document.createElement('div');d.id='shareStoryDialog';d.className='share-dialog hidden';d.innerHTML=`<div class="share-card" role="dialog" aria-modal="true" aria-labelledby="shareTitle"><button class="share-close" type="button" aria-label="Close">×</button><div class="share-kicker">SHARE THE MAGIC</div><h2 id="shareTitle">Share this story</h2><p>Send a private copy of this story to family or friends.</p><label>Your name<input id="shareSenderName" maxlength="80" autocomplete="name" placeholder="e.g. Mum, Sarah, Uncle James"></label><div id="shareRecipients"></div><button class="secondary share-add" type="button">+ Add another recipient</button><div id="shareStatus" class="share-status"></div><button class="primary share-send" type="button">Send Story</button><div id="existingShares" class="existing-shares"></div></div>`;document.body.appendChild(d);
 d.querySelector('.share-close').onclick=()=>d.classList.add('hidden');d.addEventListener('click',e=>{if(e.target===d)d.classList.add('hidden')});d.querySelector('.share-add').onclick=()=>addShareRecipient();d.querySelector('.share-send').onclick=sendStoryShares;return d
}
function addShareRecipient(name='',email=''){const box=$('shareRecipients');if(!box||box.children.length>=10)return;const row=document.createElement('div');row.className='share-recipient';row.innerHTML=`<input class="share-recipient-name" maxlength="80" placeholder="Recipient name" value="${escapeHtml(name)}"><input class="share-recipient-email" type="email" placeholder="Email address" value="${escapeHtml(email)}"><button type="button" class="share-remove" aria-label="Remove">×</button>`;row.querySelector('.share-remove').onclick=()=>{if(box.children.length>1)row.remove()};box.appendChild(row)}
async function openShareStory(){if(!currentUser){alert('Sign in to share a story.');return}const d=ensureShareDialog(),sender=$('shareSenderName');sender.value=localStorage.getItem('moonbeamShareSenderName')||'';$('shareRecipients').innerHTML='';addShareRecipient();$('shareStatus').textContent=currentBook?.isSaved?'':'Moonbeam will save this illustrated story permanently before sending it.';d.classList.remove('hidden');await refreshExistingShares()}
async function ensureCurrentBookSaved(){if(currentBook?.isSaved&&currentBook.savedStoryId)return currentBook.savedStoryId;const id=await saveCurrentStory();if(!id||!currentBook?.savedStoryId)throw new Error('The story must be saved before it can be shared.');return currentBook.savedStoryId}
async function shareApi(path,options={}){const token=await currentAccessToken();if(!token)throw new Error('Please sign in again.');const headers={...(options.headers||{}),Authorization:`Bearer ${token}`};const r=await fetch(path,{...options,headers});const raw=await r.text();let data={};try{data=JSON.parse(raw)}catch{}if(!r.ok)throw new Error(data?.error||`Sharing failed (${r.status})`);return data}
async function sendStoryShares(){const button=document.querySelector('.share-send'),status=$('shareStatus'),senderName=$('shareSenderName')?.value.trim(),recipients=[...document.querySelectorAll('.share-recipient')].map(r=>({name:r.querySelector('.share-recipient-name').value.trim(),email:r.querySelector('.share-recipient-email').value.trim()})).filter(x=>x.name||x.email);if(!senderName){status.textContent='Enter the name you want recipients to see.';return}if(!recipients.length){status.textContent='Add at least one recipient.';return}button.disabled=true;status.textContent='Preparing your private story links…';try{const storyId=await ensureCurrentBookSaved();localStorage.setItem('moonbeamShareSenderName',senderName);const data=await shareApi('/api/share?action=owner',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({storyId,senderName,recipients})});status.textContent=data.failed?.length?`Sent to ${data.sent.length}. ${data.failed.length} could not be sent.`:`Sent to ${data.sent.length} recipient${data.sent.length===1?'':'s'} ✨`;await refreshExistingShares()}catch(e){status.textContent=e.message||String(e)}finally{button.disabled=false}}
async function refreshExistingShares(){const box=$('existingShares');if(!box)return;if(!currentBook?.savedStoryId){box.innerHTML='';return}try{const data=await shareApi(`/api/share?action=owner&storyId=${encodeURIComponent(currentBook.savedStoryId)}`);if(!data.shares?.length){box.innerHTML='';return}box.innerHTML=`<h3>Shared with</h3>${data.shares.map(x=>`<div class="existing-share"><span><b>${escapeHtml(x.recipient_name)}</b><small>${escapeHtml(x.recipient_email)}${x.opened_at?' · opened':''}${x.revoked_at?' · revoked':''}</small></span>${x.revoked_at?'':`<button class="secondary" type="button" data-revoke-share="${escapeHtml(x.id)}">Revoke</button>`}</div>`).join('')}`;box.querySelectorAll('[data-revoke-share]').forEach(b=>b.onclick=async()=>{if(!confirm('Revoke this private story link?'))return;try{await shareApi('/api/share?action=owner',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:b.dataset.revokeShare})});await refreshExistingShares()}catch(e){alert(e.message)}})}catch(e){box.innerHTML=`<small>${escapeHtml(e.message)}</small>`}}
