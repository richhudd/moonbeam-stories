...
async function chooseChildPhoto(file){if(!file)return;const status=$('photoStatus');try{if(!/^image\/(jpeg|png|webp)$/i.test(file.type))throw new Error('Please choose a JPG, PNG or WebP photo.');if(status)status.textContent='Preparing photo…';const image=await resizeChildPhoto(file);currentChildPhoto=image;await childPhotoPut(currentPhotoKey(),image);if(activeProfileId)await uploadCloudChildPhoto(activeProfileId,image);if($('useChildPhoto'))$('useChildPhoto').checked=true;renderChildPhoto();renderDesktopProfileTiles();if(status)status.textContent='Photo ready for illustrations.'}catch(e){if(status)status.innerHTML=`<span class="error">${escapeHtml(e.message||e)}</span>`}}
async function removeChildPhoto(){await childPhotoDelete(currentPhotoKey());if(activeProfileId)await deleteCloudChildPhoto(activeProfileId);currentChildPhoto=null;renderChildPhoto();renderDesktopProfileTiles();if($('photoStatus'))$('photoStatus').textContent='Photo removed.'}
...
const childPhotoDrop=$('childPhotoPreview');
if(childPhotoDrop){
  ['dragenter','dragover'].forEach(type=>childPhotoDrop.addEventListener(type,e=>{if(window.matchMedia('(pointer:fine)').matches){e.preventDefault();e.dataTransfer.dropEffect='copy';childPhotoDrop.classList.add('drag-over')}}));
  ['dragleave','dragend'].forEach(type=>childPhotoDrop.addEventListener(type,e=>{if(window.matchMedia('(pointer:fine)').matches){e.preventDefault();childPhotoDrop.classList.remove('drag-over')}}));
  childPhotoDrop.addEventListener('drop',e=>{if(!window.matchMedia('(pointer:fine)').matches)return;e.preventDefault();childPhotoDrop.classList.remove('drag-over');const f=e.dataTransfer?.files?.[0];if(f)chooseChildPhoto(f)});
  childPhotoDrop.addEventListener('click',()=>{if(window.matchMedia('(pointer:fine)').matches)$('childPhotoInput')?.click()});
}
...
async function applyAuthSession(session){
 ...
 if(currentUser){
   ...
   await loadCastMembers246();await loadCloudProfiles();await Promise.all([loadCloudStories(),loadStoryCredits()]);
...
async function renderDesktopProfileTiles(){
 ...
 // V171: render the selector immediately with explicit loading avatars while local/cloud photos resolve.
 const loadingTiles=profiles.map(p=>{const selected=p.id===activeProfileId;return `<button type="button" class="desktop-child-tile photo-loading${selected?' selected':''}" data-profile-id="${escapeHtml(p.id)}" aria-pressed="${selected?'true':'false'}"><span class="desktop-child-avatar"><span class="child-photo-loading-spinner" aria-hidden="true"></span></span><span class="desktop-child-name">${escapeHtml(p.name||'Child')}</span></button>`}).join('');
 strip.innerHTML=loadingTiles+`<button type="button" class="desktop-child-tile desktop-new-child${!activeProfileId?' selected':''}" data-profile-id="" aria-pressed="${!activeProfileId?'true':'false'}"><span class="desktop-child-avatar desktop-new-child-icon">＋</span><span class="desktop-child-name">${escapeHtml(t().newChild||'New child')}</span></button>`;
...
async function loadCloudProfiles(){
 if(!currentUser)return;const {data,error}=await supabaseClient.from('cast_members').select('id,name,age,created_at').eq('kind','child').order('created_at',{ascending:true});if(error){$('profileStatus').innerHTML=`<span class="error">${escapeHtml(error.message)}</span>`;return}cloudProfiles=data||[];if(activeProfileId&&!cloudProfiles.some(p=>p.id===activeProfileId))activeProfileId=null;
...
async function saveChildProfile(){
 ...
 const profileData={name:c.name,age:c.age,kind:'child',relationship:null,animal_type:null};
 let result;if(activeProfileId)result=await supabaseClient.from('cast_members').update(profileData).eq('id',activeProfileId).select().single();else result=await supabaseClient.from('cast_members').insert({...profileData,parent_id:currentUser.id}).select().single();
 if(result.error){$('profileStatus').innerHTML=`<span class="error">${escapeHtml(result.error.message)}</span>`;if($('profileStatusBasics'))$('profileStatusBasics').innerHTML=`<span class="error">${escapeHtml(result.error.message)}</span>`;return}const wasNew=!activeProfileId;const draftKey=currentPhotoKey(null);activeProfileId=result.data.id;if(wasNew&&currentChildPhoto){await childPhotoPut(currentPhotoKey(activeProfileId),currentChildPhoto);await childPhotoDelete(draftKey);await uploadCloudChildPhoto(activeProfileId,currentChildPhoto)}await loadCastMembers246();await loadCloudProfiles();$('profileSelect').value=activeProfileId;renderProfileSelect();$('profileStatus').textContent=t().profileSaved;if($('profileStatusBasics'))$('profileStatusBasics').textContent=t().profileSaved;
}
async function ensureCloudProfile(child){
 if(!currentUser)return null;if(activeProfileId&&cloudProfiles.some(p=>p.id===activeProfileId)){const {error}=await supabaseClient.from('cast_members').update({name:child.name,age:child.age,kind:'child'}).eq('id',activeProfileId);if(!error)return activeProfileId}
 // V180: a newly-created profile starts with an empty legacy interests field; the Story idea is never persisted here.
 const draftKey=currentPhotoKey(null);const {data,error}=await supabaseClient.from('cast_members').insert({parent_id:currentUser.id,kind:'child',name:child.name,age:child.age,relationship:null,animal_type:null}).select('id').single();if(error)throw error;activeProfileId=data.id;if(currentChildPhoto){await childPhotoPut(currentPhotoKey(activeProfileId),currentChildPhoto);await childPhotoDelete(draftKey);await uploadCloudChildPhoto(activeProfileId,currentChildPhoto)}await loadCastMembers246();await loadCloudProfiles();return data.id
}
async function deleteChildProfile(){
 if(!currentUser||!activeProfileId)return;const doomed=cloudProfiles.find(p=>p.id===activeProfileId);const deleteProfileText=t().deleteProfile;const confirmText=typeof deleteProfileText==='function'?deleteProfileText(doomed?.name):`Delete ${doomed?.name||'this child'}’s profile? Saved stories will remain in your library.`;if(!confirm(confirmText))return;const doomedProfileId=activeProfileId,oldPhotoKey=currentPhotoKey(activeProfileId);const {error}=await supabaseClient.from('cast_members').delete().eq('id',activeProfileId);if(error){$('profileStatus').innerHTML=`<span class="error">${escapeHtml(error.message)}</span>`;return}await childPhotoDelete(oldPhotoKey);await deleteCloudChildPhoto(doomedProfileId);activeProfileId=null;await loadCastMembers246();await loadCloudProfiles();await selectCloudProfile();$('profileStatus').textContent=t().profileDeleted
}
...
async function loadIllustration(index,prompt,silent=false,force=false){
 ...
 try{
   ...
   let image=await requestIllustration(key,prompt,`Character continuity only: ${book.character_bible||'Keep the main child character visually consistent across the book.'}`,force,book.child?.referencePhoto||null,true,index);
...
function closeCastEditor246(){const b=$('castEditorSave');if(b){b.disabled=false;b.textContent=castText246().save;b.removeAttribute('aria-busy');b.classList.remove('saving-state','saved-state')}$('castEditor')?.classList.add('hidden');castEditor246={kind:'child',id:null,photo:null,existingPhoto:false}}
async function saveCastEditor246(){
 ...
 try{
  ...
  const payload={parent_id:currentUser.id,kind,name,age:kind==='child'?age:null,relationship:kind==='adult'?relationship:null,animal_type:kind==='pet'?animal:null};
  const r=id
  ...
  id=r.data.id;if(kind==='child')activeProfileId=id;
  ...
  if(kind==='child'){
   await loadCastMembers246();await loadCloudProfiles();
  ...
  status.textContent=`${castText246().saveError||'Could not save'}: ${err?.message||'Unable to save this cast member.'}`;
 }finally{
  button.disabled=false;button.removeAttribute('aria-busy');
  if(!$('castEditor')?.classList.contains('hidden'))button.textContent=originalLabel;
 }
}
async function deleteCastMember246(kind,id,name){const x=castText246();if(!confirm(x.confirmDelete(name)))return;const{error}=await supabaseClient.from('cast_members').delete().eq('id',id);if(error){$('castStatus').textContent=error.message;return}await childPhotoDelete(currentPhotoKey(id));await deleteCloudChildPhoto(id);if(activeProfileId===id)activeProfileId=null;await loadCastMembers246();await loadCloudProfiles();await renderCast246();$('castStatus').textContent=x.deleted}
