...
function applyLocale(){
 ...
 renderValues(); renderLibrary(); renderStoryCredits(); applyInterfaceLocale(); applyLandingLocale();
}
...
function ensureShareDialog(){
 let d=$('shareStoryDialog');if(!d){d=document.createElement('div');d.id='shareStoryDialog';d.className='share-dialog hidden';d.innerHTML=`<div class="share-card" role="dialog" aria-modal="true" aria-labelledby="shareTitle"><button class="share-close" type="button">×</button><div class="share-kicker"></div><h2 id="shareTitle"></h2><p class="share-intro"></p><label class="share-sender-label"><span></span><input id="shareSenderName" maxlength="80" autocomplete="name"></label><div id="shareRecipients"></div><button class="secondary share-add" type="button"></button><div id="shareStatus" class="share-status"></div><button class="primary share-send" type="button"></button><div id="existingShares" class="existing-shares"></div></div>`;document.body.appendChild(d);d.querySelector('.share-close').onclick=()=>d.classList.add('hidden');d.addEventListener('click',e=>{if(e.target===d)d.classList.add('hidden')});d.querySelector('.share-add').onclick=()=>addShareRecipient();d.querySelector('.share-send').onclick=sendStoryShares}
 applyShareDialogLocale(d);return d
}
function applyShareDialogLocale(d=$('shareStoryDialog')){if(!d)return;const sx=shareT(shareStoryLanguage());d.lang=shareStoryLanguage();d.querySelector('.share-close').setAttribute('aria-label',sx.remove);d.querySelector('.share-kicker').textContent=sx.kicker;d.querySelector('#shareTitle').textContent=sx.title;d.querySelector('.share-intro').textContent=sx.intro;d.querySelector('.share-sender-label span').textContent=sx.yourName;d.querySelector('#shareSenderName').placeholder=sx.senderPh;d.querySelector('.share-add').textContent=sx.add;d.querySelector('.share-send').textContent=sx.send;d.querySelectorAll('.share-recipient').forEach(applyShareRecipientLocale)}
...
function addShareRecipient(email=''){const box=$('shareRecipients');if(!box||box.children.length>=10)return;const row=document.createElement('div');row.className='share-recipient';row.innerHTML=`<input class="share-recipient-email" type="email" value="${escapeHtml(email)}"><button type="button" class="share-remove">×</button>`;row.querySelector('.share-remove').onclick=()=>{if(box.children.length>1)row.remove()};box.appendChild(row);applyShareRecipientLocale(row)}
async function openShareStory(){const sx=shareT(shareStoryLanguage());if(!currentUser){alert(sx.signIn);return}const d=ensureShareDialog(),sender=$('shareSenderName');applyShareDialogLocale(d);sender.value=localStorage.getItem('moonbeamShareSenderName')||'';$('shareRecipients').innerHTML='';addShareRecipient();$('shareStatus').textContent=currentBook?.isSaved?'':sx.willSave;d.classList.remove('hidden');await refreshExistingShares()}
