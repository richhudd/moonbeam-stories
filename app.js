...
function coverKey(book){return `v47:${book.visualCacheId||book.cacheId}:cover`}
function getCoverPrompt(book){
 const first=book.pages[0]?.illustration_prompt||'';
 return `Front cover illustration for an original premium children's adventure called “${book.title}”. Main child/hero: ${book.child?.name||'the child'}, age ${book.child?.age||7}. Story idea: ${book.child?.storyIdea||'an original Moonbeam adventure'}. Story world and character continuity: ${book.character_bible||'Keep the hero appealing and visually consistent with the interior illustrations.'} Opening: ${book.opening||''} Visual clue from the first scene: ${first}. Compose this specifically as a strong children's BOOK COVER: one clear focal character and a composition appropriate to the story's actual action, setting and time of day. The server's fixed Moonbeam house style is authoritative; this cover direction describes composition and content only and must not alter the rendering style or realism level. Keep the central and upper areas sufficiently calm and uncluttered for title typography that will be overlaid by the app. Absolutely no words, letters, captions, logos, signs or readable text anywhere in the image. Follow the story's actual setting and time of day; do not default to nighttime, moonlight, stars, darkness, sleep or bedtime imagery unless the story itself specifically calls for them.`
}
...
function illustrationKey(book,index,prompt=''){return `v47:${book.visualCacheId||book.cacheId}:${index}:${stableHash(String(prompt||''))}`}
...
async function loadIllustration(index,prompt,silent=false,force=false){
 ...
 try{
   ...
   let image=await requestIllustration(key,prompt,`Story visual continuity bible: ${book.character_bible||'Keep recurring characters, locations and objects visually consistent across the book.'}`,force,book.child?.referenceImages||book.child?.referencePhoto||null,true,index,prevImage);
