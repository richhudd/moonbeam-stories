...
function renderLibrary(){
 ...
 if(currentUser){
   l.innerHTML=`<div class="saved-library-note">📚 ${escapeHtml(c.readOnly)} ${escapeHtml(c.free)}</div><div class="saved-story-list">${items.map((x,i)=>`<article class="saved-story-card"><div class="saved-story-main"><div class="saved-story-title">${escapeHtml(x.title)}</div><small>${escapeHtml(x.child?.name||'')}</small></div><div class="saved-story-actions"><button class="primary saved-read" type="button" onclick="openSaved(${i})">📖 ${escapeHtml(c.open)}</button><label class="saved-language-compact"><span>${escapeHtml(c.readIn)}</span><select id="savedLang${i}" onchange="updateSavedAction(${i})">${opts(x)}</select></label><button class="secondary saved-translate" type="button" onclick="openOrTranslateSaved(${i})">🌐 ${escapeHtml(c.open)}</button><button class="library-delete" type="button" onclick="deleteSavedStory('${escapeHtml(x.id)}')">🗑 ${escapeHtml(t().delete)}</button></div></article>`).join('')}</div>`;
 }else{
   l.innerHTML=items.map((x,i)=>`<div class="library-item"><button type="button" onclick="openSaved(${i})">📖 ${escapeHtml(x.title)} <small>— ${escapeHtml(x.child?.name||'')}</small></button><small>${escapeHtml(c.signIn)}</small></div>`).join('');
 }
}
