...
function renderLibrary(){
 ...
 if(currentUser){l.innerHTML=items.map((x,i)=>`<div class="library-item"><button class="library-open" type="button" onclick="openSaved(${i})">📖 ${escapeHtml(x.title)} <small>— ${escapeHtml(x.child?.name||'')}</small></button><button class="library-delete" type="button" onclick="deleteSavedStory('${escapeHtml(x.id)}')">${escapeHtml(t().delete)}</button></div>`).join('')}else{l.innerHTML=items.map((x,i)=>`<button type="button" onclick="openSaved(${i})">📖 ${escapeHtml(x.title)} <small>— ${escapeHtml(x.child?.name||'')}</small></button>`).join('')}
}
