...
async function requestIllustration(key,prompt,style,force=false,referenceImage=null,requiredStoryImage=false,storyImageIndex=null,referenceImages=null){
 ...
 const task=(async()=>{let attempts=0;while(attempts<3){attempts++;const accessToken=await currentAccessToken();if(!accessToken)throw new Error('Sign in again to create illustrations.');const generationRunId=currentBook?.generationRunId||null;if(!generationRunId)throw new Error('This saved story predates the secure illustration allowance.');const response=await fetch('/api/illustrate',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${accessToken}`},body:JSON.stringify({prompt,style,referenceImage:referenceImage||null,referenceImages:Array.isArray(referenceImages)?referenceImages:null,generationRunId,requiredStoryImage:requiredStoryImage===true,storyImageIndex:Number.isInteger(storyImageIndex)?storyImageIndex:null})});const raw=await response.text();let data=null;try{data=JSON.parse(raw)}catch{}if(response.ok&&data?.image){
   if((referenceImage||(Array.isArray(referenceImages)&&referenceImages.length)) && data.usedReferencePhoto!==true) throw new Error('The child photo reference was not accepted by the illustration service.');
...
async function generateStory(){
 ...
 const cast=await selectedStoryCast246();
 if(!cast.heroes.length){$('status').textContent='Choose at least one hero.';return}
 const primaryHero=cast.heroes[0];
 const child={name:primaryHero.name,age:Number(primaryHero.age),storyIdea:($('desktopStoryIdea')?.value||'').trim(),dislikes:$('dislikes').value.trim(),length:'standard',tone:$('tone').value,language,languageName:languageNames[language],values:[...selected],profileId:primaryHero.id||null,referencePhoto:primaryHero.referencePhoto||null,cast};
 ...
 try{
   const response=await fetch('/api/generate',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${accessToken}`},body:JSON.stringify({child,cast})});
 ...
 if(isPhonePortrait()){
   try{
     ...
     const openingImage=await requestIllustration(
       ...
       true,
       0,
       castReferences246(book.child?.cast)
     );
     if(currentBook===book){
...
async function loadIllustration(index,prompt,silent=false,force=false){
 ...
 try{
   ...
   let image=await requestIllustration(key,prompt,`Character continuity only: ${book.character_bible||'Keep the main child character visually consistent across the book.'}`,force,book.child?.referencePhoto||null,true,index,castReferences246(book.child?.cast));
...
function goSetupPage(index,instant=false){
 ...
 if(currentUser&&(setupPageIndex===storyIndex||pages[setupPageIndex]?.dataset.step==='Child'))refreshStoryCreditConsentUI().catch(()=>{});
 updateSetupNav();
 bindSetupScrollCue();
...
// V245 — simple Cast management. Children remain in child_profiles; adults/pets use cast_members.
let castMembers245=[],castModalRole245='adult',castEditingId245=null,castPendingPhoto245=null;
let storyHeroIds246=new Set(),storySupportingKeys246=new Set();
const castPhotoCache245=new Map();
function castFolder245(id){return currentUser&&id?`${currentUser.id}/cast-${id}`:null}
async function castPhotoGet245(id){if(castPhotoCache245.has(id))return castPhotoCache245.get(id);const folder=castFolder245(id);if(!folder||!supabaseClient)return null;try{const {data,error}=await supabaseClient.storage.from(CHILD_PHOTO_BUCKET).list(folder,{limit:10,sortBy:{column:'created_at',order:'desc'}});if(error||!data?.length)return null;const path=`${folder}/${data[0].name}`;const {data:blob,error:down}=await supabaseClient.storage.from(CHILD_PHOTO_BUCKET).download(path);if(down||!blob)return null;const image=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(blob)});castPhotoCache245.set(id,image);return image}catch{return null}}
async function castPhotoDelete245(id){const folder=castFolder245(id);if(!folder||!supabaseClient)return;try{const {data}=await supabaseClient.storage.from(CHILD_PHOTO_BUCKET).list(folder,{limit:20});if(data?.length)await supabaseClient.storage.from(CHILD_PHOTO_BUCKET).remove(data.map(x=>`${folder}/${x.name}`))}catch{}castPhotoCache245.delete(id)}
async function castPhotoUpload245(id,image){if(!image)return;await castPhotoDelete245(id);const folder=castFolder245(id);if(!folder)return;const path=`${folder}/reference-${Date.now()}.jpg`;const {error}=await supabaseClient.storage.from(CHILD_PHOTO_BUCKET).upload(path,dataUrlToBlob(image),{contentType:'image/jpeg',upsert:false});if(error)throw error;castPhotoCache245.set(id,image)}
function castCard245(member,photo,kind){const child=kind==='child',selected=child&&member.id===activeProfileId;const detail=child?`Age ${member.age}`:(member.detail|| (kind==='pet'?'Pet':'Supporting role'));return `<article class="cast-card${selected?' selected':''}" data-cast-kind="${kind}" data-cast-id="${escapeHtml(member.id)}"><button class="cast-card-photo" type="button" data-cast-select="1" aria-label="Select ${escapeHtml(member.name)}">${photo?`<img src="${photo}" alt="">`:'☾'}</button><div class="cast-card-copy"><strong>${escapeHtml(member.name)}</strong><small>${escapeHtml(detail)}</small>${selected?'<small>Main character for tonight</small>':''}</div><button class="cast-menu" type="button" aria-label="Manage">⋯</button><div class="cast-menu-pop hidden"></div></article>`}
async function renderCast245(){const children=$('castChildren'),adults=$('castAdults'),pets=$('castPets');if(!children||!adults||!pets)return;const cps=await Promise.all(cloudProfiles.map(p=>childPhotoGetForProfile(p.id)));children.innerHTML=cloudProfiles.map((p,i)=>castCard245(p,cps[i],'child')).join('')||'<p class="muted">Add a child to begin.</p>';const adultsList=castMembers245.filter(x=>x.role==='adult'),petsList=castMembers245.filter(x=>x.role==='pet');const ap=await Promise.all(adultsList.map(x=>castPhotoGet245(x.id))),pp=await Promise.all(petsList.map(x=>castPhotoGet245(x.id)));adults.innerHTML=adultsList.map((x,i)=>castCard245(x,ap[i],'adult')).join('')||'<p class="muted">No supporting adults added.</p>';pets.innerHTML=petsList.map((x,i)=>castCard245(x,pp[i],'pet')).join('')||'<p class="muted">No pets added.</p>';bindCastCards245();await renderStoryRoles246()}
function bindCastCards245(){document.querySelectorAll('.cast-card').forEach(card=>{const kind=card.dataset.castKind,id=card.dataset.castId,menu=card.querySelector('.cast-menu'),pop=card.querySelector('.cast-menu-pop');card.querySelector('[data-cast-select]')?.addEventListener('click',async()=>{if(kind==='child'){const sel=$('profileSelect');if(sel)sel.value=id;await selectCloudProfile();await renderCast245();$('childCastEditor')?.scrollIntoView({behavior:'smooth',block:'nearest'})}else openCastModal245(kind,id)});menu?.addEventListener('click',async e=>{e.stopPropagation();document.querySelectorAll('.cast-menu-pop').forEach(x=>{if(x!==pop)x.classList.add('hidden')});const hasPhoto=kind==='child'?!!(await childPhotoGetForProfile(id)):!!(await castPhotoGet245(id));pop.innerHTML=`<button data-action="photo">${hasPhoto?'Change photo':'Add photo'}</button>${hasPhoto?'<button data-action="remove-photo">Remove photo</button>':''}<button class="danger-item" data-action="delete">Delete from Cast</button>`;pop.classList.toggle('hidden');pop.querySelectorAll('button').forEach(b=>b.onclick=async()=>{pop.classList.add('hidden');const action=b.dataset.action;if(kind==='child'){const sel=$('profileSelect');if(sel)sel.value=id;await selectCloudProfile();if(action==='photo')$('childPhotoInput')?.click();if(action==='remove-photo')await removeChildPhoto();if(action==='delete')await deleteChildProfile();await renderCast245()}else{if(action==='photo'){openCastModal245(kind,id);setTimeout(()=>$('castMemberPhotoInput')?.click(),50)}if(action==='remove-photo'){await castPhotoDelete245(id);await renderCast245()}if(action==='delete')await deleteCastMember245(id)}})})})}
async function loadCastMembers245(){if(!currentUser){castMembers245=[];return}try{const {data,error}=await supabaseClient.from('cast_members').select('id,role,name,detail,created_at').order('created_at',{ascending:true});if(error)throw error;castMembers245=data||[]}catch(e){console.warn('Cast members unavailable until V245 SQL is applied:',e?.message||e);castMembers245=[]}await renderCast245()}
function openCastModal245(role,id=null){castModalRole245=role;castEditingId245=id;castPendingPhoto245=null;const item=castMembers245.find(x=>x.id===id);$('castModalTitle').textContent=item?`Edit ${item.name}`:(role==='pet'?'Add pet':'Add supporting adult');$('castMemberName').value=item?.name||'';$('castMemberDetailLabel').textContent=role==='pet'?'Type of animal':'Relationship';$('castMemberDetail').placeholder=role==='pet'?'Dog':'Dad';$('castMemberDetail').value=item?.detail||'';$('castMemberDelete').classList.toggle('hidden',!item);$('castMemberStatus').textContent='';$('castModal').classList.remove('hidden');renderCastModalPhoto245(id)}
async function renderCastModalPhoto245(id){const image=castPendingPhoto245||(id?await castPhotoGet245(id):null),box=$('castMemberPhotoPreview');box.innerHTML=image?`<img src="${image}" alt="">`:'<span>☾</span><small>No photo</small>';$('castMemberPhotoButton').textContent=image?'Change photo':'Add photo';$('castMemberPhotoRemove').classList.toggle('hidden',!image)}
async function saveCastMember245(){if(!currentUser)return;const name=$('castMemberName').value.trim(),detail=$('castMemberDetail').value.trim();if(!name){$('castMemberStatus').textContent='Add a name first.';return}if(!detail){$('castMemberStatus').textContent=castModalRole245==='pet'?'Add the type of animal.':'Add their relationship.';return}$('castMemberStatus').textContent='Saving…';let id=castEditingId245;if(id){const {error}=await supabaseClient.from('cast_members').update({name,detail}).eq('id',id);if(error){$('castMemberStatus').textContent=error.message;return}}else{const {data,error}=await supabaseClient.from('cast_members').insert({parent_id:currentUser.id,role:castModalRole245,name,detail}).select('id').single();if(error){$('castMemberStatus').textContent=error.message;return}id=data.id}if(castPendingPhoto245)try{await castPhotoUpload245(id,castPendingPhoto245)}catch(e){$('castMemberStatus').textContent=e.message;return}await loadCastMembers245();$('castModal').classList.add('hidden')}
async function deleteCastMember245(id=castEditingId245){const item=castMembers245.find(x=>x.id===id);if(!item||!confirm(`Delete ${item.name} from your Cast?`))return;const {error}=await supabaseClient.from('cast_members').delete().eq('id',id);if(error){$('castMemberStatus').textContent=error.message;return}await castPhotoDelete245(id);castMembers245=castMembers245.filter(x=>x.id!==id);$('castModal').classList.add('hidden');await renderCast245()}
$('castAddChild')?.addEventListener('click',async()=>{activeProfileId=null;const sel=$('profileSelect');if(sel)sel.value='';await selectCloudProfile();await renderCast245();$('childCastEditor')?.scrollIntoView({behavior:'smooth',block:'center'})});$('castAddAdult')?.addEventListener('click',()=>openCastModal245('adult'));$('castAddPet')?.addEventListener('click',()=>openCastModal245('pet'));$('castModalClose')?.addEventListener('click',()=>$('castModal').classList.add('hidden'));$('castModal')?.addEventListener('click',e=>{if(e.target===$('castModal'))$('castModal').classList.add('hidden')});$('castMemberPhotoButton')?.addEventListener('click',()=>$('castMemberPhotoInput').click());$('castMemberPhotoInput')?.addEventListener('change',async e=>{const f=e.target.files?.[0];if(!f)return;try{castPendingPhoto245=await resizeChildPhoto(f);await renderCastModalPhoto245(castEditingId245)}catch(err){$('castMemberStatus').textContent=err.message}e.target.value=''});$('castMemberPhotoRemove')?.addEventListener('click',async()=>{if(castEditingId245)await castPhotoDelete245(castEditingId245);castPendingPhoto245=null;await renderCastModalPhoto245(null);await renderCast245()});$('castMemberSave')?.addEventListener('click',saveCastMember245);$('castMemberDelete')?.addEventListener('click',()=>deleteCastMember245());



function storyCastKey246(kind,id){return `${kind}:${id}`}
async function renderStoryRoles246(){
 const heroBox=$('storyHeroes'),supportBox=$('storySupporting');if(!heroBox||!supportBox)return;
 const childPhotos=await Promise.all(cloudProfiles.map(p=>childPhotoGetForProfile(p.id)));
 const validChildIds=new Set(cloudProfiles.map(p=>p.id));storyHeroIds246=new Set([...storyHeroIds246].filter(id=>validChildIds.has(id)));
 if(!storyHeroIds246.size&&activeProfileId&&validChildIds.has(activeProfileId))storyHeroIds246.add(activeProfileId);
 if(!storyHeroIds246.size&&cloudProfiles[0])storyHeroIds246.add(cloudProfiles[0].id);
 const choice=(kind,id,name,detail,photo,selected)=>`<button type="button" class="story-role-choice${selected?' selected':''}" data-role-kind="${kind}" data-role-id="${escapeHtml(id)}" aria-pressed="${selected?'true':'false'}"><span class="story-role-choice-avatar">${photo?`<img src="${photo}" alt="">`:'☾'}</span><span><strong>${escapeHtml(name)}</strong><small>${escapeHtml(detail)}</small></span></button>`;
 heroBox.innerHTML=cloudProfiles.map((c,i)=>choice('hero',c.id,c.name,`Age ${c.age}`,childPhotos[i],storyHeroIds246.has(c.id))).join('')||'<p class="muted">Add a child to choose a hero.</p>';
 const supporting=[];
 cloudProfiles.forEach((c,i)=>{if(!storyHeroIds246.has(c.id))supporting.push({kind:'child',id:c.id,name:c.name,detail:`Child · age ${c.age}`,photo:childPhotos[i]})});
 for(const m of castMembers245){supporting.push({kind:m.role,id:m.id,name:m.name,detail:m.role==='pet'?m.detail:`${m.detail} · adult`,photo:await castPhotoGet245(m.id)})}
 const validKeys=new Set(supporting.map(x=>storyCastKey246(x.kind,x.id)));storySupportingKeys246=new Set([...storySupportingKeys246].filter(k=>validKeys.has(k)));
 supportBox.innerHTML=supporting.map(x=>choice(x.kind,x.id,x.name,x.detail,x.photo,storySupportingKeys246.has(storyCastKey246(x.kind,x.id)))).join('')||'<p class="muted">No other Cast members yet.</p>';
 heroBox.querySelectorAll('.story-role-choice').forEach(b=>b.onclick=async()=>{const id=b.dataset.roleId;if(storyHeroIds246.has(id)){if(storyHeroIds246.size>1)storyHeroIds246.delete(id)}else storyHeroIds246.add(id);storySupportingKeys246.delete(storyCastKey246('child',id));await renderStoryRoles246()});
 supportBox.querySelectorAll('.story-role-choice').forEach(b=>b.onclick=async()=>{const key=storyCastKey246(b.dataset.roleKind,b.dataset.roleId);storySupportingKeys246.has(key)?storySupportingKeys246.delete(key):storySupportingKeys246.add(key);await renderStoryRoles246()});
}
async function selectedStoryCast246(){
 const heroes=[];for(const c of cloudProfiles){if(storyHeroIds246.has(c.id))heroes.push({id:c.id,kind:'child',role:'hero',name:c.name,age:Number(c.age),referencePhoto:await childPhotoGetForProfile(c.id)})}
 const supporting=[];
 for(const c of cloudProfiles){if(storySupportingKeys246.has(storyCastKey246('child',c.id)))supporting.push({id:c.id,kind:'child',role:'supporting',name:c.name,age:Number(c.age),referencePhoto:await childPhotoGetForProfile(c.id)})}
 for(const m of castMembers245){if(storySupportingKeys246.has(storyCastKey246(m.role,m.id)))supporting.push({id:m.id,kind:m.role,role:'supporting',name:m.name,detail:m.detail,referencePhoto:await castPhotoGet245(m.id)})}
 return {heroes,supporting};
}
function castReferences246(cast){return [...(cast?.heroes||[]),...(cast?.supporting||[])].filter(x=>x.referencePhoto).map(x=>({image:x.referencePhoto,label:x.kind==='child'?`${x.name} — child, age ${x.age} — ${x.role==='hero'?'HERO':'SUPPORTING CHILD'}`:x.kind==='adult'?`${x.name} — adult, ${x.detail} — SUPPORTING ADULT`:`${x.name} — ${x.detail} — PET / COMPANION`}))}

const _loadCloudProfiles245=loadCloudProfiles;loadCloudProfiles=async function(){const r=await _loadCloudProfiles245.apply(this,arguments);await renderCast245();return r};
const _saveChildProfile245=saveChildProfile;saveChildProfile=async function(){const r=await _saveChildProfile245.apply(this,arguments);await renderCast245();return r};
const _deleteChildProfile245=deleteChildProfile;deleteChildProfile=async function(){const r=await _deleteChildProfile245.apply(this,arguments);await renderCast245();return r};
