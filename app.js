...
async function generateStory(){
 ...
 const cast=await storyCastPayload248();const heroes=cast.filter(m=>m.role==='hero');if(!heroes.length){$('storyRoleStatus').textContent=storyRoleText248().needHero;return}else $('storyRoleStatus').textContent='';const primary=heroes[0];const child={name:primary.name,age:Math.min(...heroes.map(h=>Number(h.age)||12)),storyIdea:($('desktopStoryIdea')?.value||'').trim(),dislikes:'',length:'standard',language,languageName:languageNames[language],profileId:primary.id||null,referencePhoto:primary.referencePhoto||resolvedReferencePhoto||null,referenceImages:cast.filter(m=>m.referencePhoto).map(m=>({name:m.name,kind:m.kind,role:m.role,image:m.referencePhoto})),cast};
...
'en-GB':{heroes:"Who’s this story about?",heroSub:'Choose one or more heroes',support:'Anyone else Moonbeam may use?',supportSub:'Optional familiar people or pets — Moonbeam will use them only if the story naturally needs them.',needHero:'Choose at least one hero.',age:a=>`Age ${a}`,breed:'Breed (optional)',idea:'Optional: a word, theme, place, funny thought or something that happened — or leave blank and let Moonbeam surprise you.'},
'en-US':{heroes:"Who’s this story about?",heroSub:'Choose one or more heroes',support:'Anyone else Moonbeam may use?',supportSub:'Optional familiar people or pets — Moonbeam will use them only if the story naturally needs them.',needHero:'Choose at least one hero.',age:a=>`Age ${a}`,breed:'Breed (optional)',idea:'Optional: a word, theme, place, funny thought or something that happened — or leave blank and let Moonbeam surprise you.'},
'es-ES':{heroes:'¿De quién trata esta historia?',heroSub:'Elige uno o más protagonistas',support:'¿Alguien más que Moonbeam pueda usar?',supportSub:'Personas o mascotas conocidas opcionales; Moonbeam solo las usará si la historia las necesita de forma natural.',needHero:'Elige al menos un protagonista.',age:a=>`Edad ${a}`,breed:'Raza (opcional)',idea:'Opcional: una palabra, tema, lugar, idea divertida o algo que haya pasado; o déjalo en blanco y deja que Moonbeam te sorprenda.'},
'es-419':{heroes:'¿De quién trata esta historia?',heroSub:'Elige uno o más protagonistas',support:'¿Alguien más que Moonbeam pueda usar?',supportSub:'Personas o mascotas conocidas opcionales; Moonbeam solo las usará si la historia las necesita de forma natural.',needHero:'Elige al menos un protagonista.',age:a=>`Edad ${a}`,breed:'Raza (opcional)',idea:'Opcional: una palabra, tema, lugar, idea divertida o algo que haya pasado; o déjalo en blanco y deja que Moonbeam te sorprenda.'},
'fr-FR':{heroes:"De qui parle cette histoire ?",heroSub:'Choisissez un ou plusieurs héros',support:"Quelqu’un d’autre que Moonbeam peut utiliser ?",supportSub:"Personnes ou animaux familiers facultatifs — Moonbeam ne les utilisera que si l’histoire en a naturellement besoin.",needHero:'Choisissez au moins un héros.',age:a=>`Âge ${a}`,breed:'Race (facultatif)',idea:"Facultatif : un mot, un thème, un lieu, une idée amusante ou quelque chose qui s’est passé — ou laissez vide et laissez Moonbeam vous surprendre."},
'de-DE':{heroes:'Um wen geht es in dieser Geschichte?',heroSub:'Wähle eine oder mehrere Hauptfiguren',support:'Wen darf Moonbeam sonst noch verwenden?',supportSub:'Optionale vertraute Personen oder Tiere — Moonbeam verwendet sie nur, wenn die Geschichte sie natürlich braucht.',needHero:'Wähle mindestens eine Hauptfigur.',age:a=>`Alter ${a}`,breed:'Rasse (optional)',idea:'Optional: ein Wort, Thema, Ort, lustiger Gedanke oder etwas, das passiert ist — oder leer lassen und von Moonbeam überraschen lassen.'},
'it-IT':{heroes:'Di chi parla questa storia?',heroSub:'Scegli uno o più protagonisti',support:'Chi altro può usare Moonbeam?',supportSub:'Persone o animali familiari facoltativi — Moonbeam li userà solo se la storia ne ha naturalmente bisogno.',needHero:'Scegli almeno un protagonista.',age:a=>`Età ${a}`,breed:'Razza (facoltativa)',idea:'Facoltativo: una parola, un tema, un luogo, un’idea buffa o qualcosa che è successo — oppure lascia vuoto e fatti sorprendere da Moonbeam.'},
'pt-BR':{heroes:'Sobre quem é esta história?',heroSub:'Escolha um ou mais protagonistas',support:'Quem mais o Moonbeam pode usar?',supportSub:'Pessoas ou animais conhecidos opcionais — o Moonbeam só os usará se a história precisar deles naturalmente.',needHero:'Escolha pelo menos um protagonista.',age:a=>`Idade ${a}`,breed:'Raça (opcional)',idea:'Opcional: uma palavra, tema, lugar, ideia engraçada ou algo que aconteceu — ou deixe em branco e deixe o Moonbeam surpreender você.'},
'pl-PL':{heroes:'O kim jest ta historia?',heroSub:'Wybierz jednego lub kilku głównych bohaterów',support:'Kogo jeszcze Moonbeam może wykorzystać?',supportSub:'Opcjonalne znajome osoby lub zwierzęta — Moonbeam użyje ich tylko wtedy, gdy historia naturalnie ich potrzebuje.',needHero:'Wybierz co najmniej jednego głównego bohatera.',age:a=>`Wiek ${a}`,breed:'Rasa (opcjonalnie)',idea:'Opcjonalnie: słowo, temat, miejsce, zabawny pomysł lub coś, co się wydarzyło — albo zostaw puste i pozwól Moonbeam Cię zaskoczyć.'}
};
...
async function renderStoryRoles248(){
 ...
 const choice=async(m,role)=>{const photo=await castPhoto246(m.id),active=role==='hero'?storyHeroIds248.has(m.id):storySupportIds248.has(m.id),meta=m.kind==='child'?x.age(m.age):m.kind==='adult'?(m.relationship||''):[m.animal_type,m.breed].filter(Boolean).join(' · ');return `<label class="story-role-choice${active?' active':''}" data-role="${role}" data-id="${escapeHtml(m.id)}"><input type="checkbox" ${active?'checked':''}><span class="role-avatar">${photo?`<img src="${photo}" alt="">`:'☾'}</span><span class="role-copy"><strong>${escapeHtml(m.name)}</strong><small>${escapeHtml(meta||'')}</small></span><span class="role-select-indicator" aria-hidden="true">${active?'✓':''}</span></label>`};
 ...
 hero.querySelectorAll('.story-role-choice').forEach(el=>el.onchange=async()=>{const id=el.dataset.id;if(el.querySelector('input').checked){storyHeroIds248.add(id);storySupportIds248.delete(id);activeProfileId=id;syncGenerationAdapter246()}else storyHeroIds248.delete(id);await renderStoryRoles248()});
 support.querySelectorAll('.story-role-choice').forEach(el=>el.onchange=async()=>{const id=el.dataset.id;if(el.querySelector('input').checked)storySupportIds248.add(id);else storySupportIds248.delete(id);await renderStoryRoles248()});
}
