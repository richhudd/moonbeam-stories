...
module.exports=async function handler(req,res){
 ...
 try{
  ...
  // Use the caller's authenticated Supabase session for saved-story reads. This deliberately
  // follows the same RLS ownership path as the browser library, so a story the signed-in
  // parent can read cannot disappear merely because a separate admin/parent_id lookup differs.
  const token=String(req.headers.authorization||'').replace(/^Bearer\s+/i,'');
  const userHeaders={apikey:PUBLISHABLE_KEY,Authorization:`Bearer ${token}`};
  const q=new URLSearchParams({id:`eq.${storyId}`,select:'id,title,language,opening,pages,closing,translations'});
  const sr=await fetch(`${SUPABASE_URL}/rest/v1/saved_stories?${q}`,{headers:userHeaders});
  ...
  const ur=await fetch(`${SUPABASE_URL}/rest/v1/saved_stories?id=eq.${encodeURIComponent(storyId)}`,{method:'PATCH',headers:{...userHeaders,'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({translations})});
