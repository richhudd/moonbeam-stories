...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const storyReferenceImages = Array.isArray(body.storyReferenceImages) ? body.storyReferenceImages.slice(0,8) : [];
    ...
    const storyRefs=(Array.isArray(storyReferenceImages)?storyReferenceImages:[]).filter(r=>r&&/^data:image\/(jpeg|png|webp);base64,/i.test(r.image||''));
    const hasReference = refs.length>0;
    const hasStoryReferences = storyRefs.length>0;
    const hasContinuityReference = Boolean(continuityImage);
    const hasAnyReference = hasReference || hasContinuityReference || hasStoryReferences;
    const identityDirection = hasReference
      ? `\nIDENTITY REFERENCES — ABSOLUTE CAST IDENTITY LOCK\nThe attached photographs are NOT generic inspiration. Each photograph is the authoritative identity reference for exactly ONE selected Cast member, mapped in attachment order as follows: ${refs.map((r,i)=>`REFERENCE ${i+1} = ${r.name||'character'} | kind: ${r.kind||'character'} | role: ${r.role||'supporting'}${r.gender?` | gender: ${r.gender}`:''}`).join('; ')}.\nFor EVERY mapped Cast member, child, adult or pet, reproduce that specific individual whenever the named character appears. Adult identity references have exactly the same priority and force as child identity references. Do not invent an alternative face or appearance for a photographed adult and do not use the real photographed adult later as a different extra person. Preserve recognisable face shape, eyes, nose, mouth anatomy, hair colour/texture, approximate skin tone, apparent age and overall physical identity. Any supplied optional male/female marker in the reference mapping is authoritative and must be preserved. Preserve the person's underlying identity even when the story setting changes: do NOT localise, ethnically reinterpret or restyle a photographed child, adult or pet to match a location, country, era or genre. The reference expression is NOT a pose that must be copied into every scene: let emotion follow the story naturally. You may safely reduce visible expression information — for example, a photographed toothy smile may become closed-mouth, neutral, serious, worried, surprised or thoughtful when the scene calls for it. But do NOT invent personal facial information that the reference does not reveal. In particular, if the reference does not show the person's teeth or an open-mouth smile, do not invent visible teeth or a broad open-mouth smile; convey happiness through a natural closed-mouth smile, cheeks, eyes and expression instead. If teeth/open-mouth smile are visible in the reference, that observed smile may be used when appropriate but is never compulsory. Do not invent decorative accessories that are not supported by the reference photo or the story. If a photographed child does not visibly wear a hair clip, bow, barrette, tiara, headband ornament, decorative star or other hair accessory in the reference, do not add one. This is especially important for people marked male: never invent hair clips, bows or similar decorative hair accessories for a male character unless the story explicitly requires them. For photographed pets preserve species/breed appearance, coat, body proportions and relative size. Translate each real identity naturally into the fixed Moonbeam painted style rather than making the result photographic. Reference background, clothing, gaze, head angle and pose are NOT identity requirements unless the scene calls for them. Never merge identities, swap faces, assign one reference to another named character, use a reference as a generic person, or create both an invented version and the real version of the same Cast member. One named Cast member = one stable visual identity across the whole book.`
      : '';
    const storyReferenceDirection = hasStoryReferences
      ? `\nCOMPLETED INTERIOR ILLUSTRATIONS — AUTHORITATIVE BOOK-WIDE VISUAL REFERENCES\nAdditional attached images after any Cast photographs are completed interior illustrations from this same Moonbeam book. Use them as authoritative references for how this specific book already renders its characters, faces, costumes, colour logic, props, locations and overall painting treatment. This is especially important when generating the cover: the cover should visibly belong to the same finished book and should match the same child likenesses and Moonbeam rendering language. Preserve the already-established facial interpretation, styling, costume continuity, object designs and world appearance. Do NOT copy one of these pages literally; instead, compose a fresh image that clearly belongs to the same book.`
      : '';
    const continuityDirection = hasContinuityReference
      ? `\nPREVIOUS-PAGE ARTWORK — AUTHORITATIVE WORLD AND OBJECT CONTINUITY REFERENCE\nThe final attached image after any Cast photographs and any completed interior illustration references is the immediately preceding page artwork. Use it to preserve the established visual identity and physical state of recurring locations, buildings, rooms, vehicles, machines, props, clothing and other plot-important objects. Keep the same number, overall structure, proportions, materials, colours, weathering, layout, openings and placement/shape of distinctive parts. One established lighthouse remains one lighthouse; one established wheel, handle or control remains that exact same component. Do not create a duplicate merely because the camera now looks from another angle. Story-required changes may occur, but otherwise the established design is immutable. This reference controls continuity only: do NOT copy its composition, camera angle, crop, poses or staging. The current scene direction controls the new action and viewpoint. Do not treat people visible in this artwork as additional identity photographs or extra Cast members.`
      : '';
...
- ACCESSORY DISCIPLINE: do not invent decorative hair accessories, hair clips, bows, barrettes, tiaras, star ornaments, jewellery pins or similar adornments unless they are clearly visible in the reference photo or explicitly required by the story. If an earlier illustration accidentally introduced such an unsupported accessory, remove it rather than preserving it.
...
- Square composition suitable for the right-hand page of a children's book.`;
    ...
    if (hasAnyReference) {
      ...
      for(let i=0;i<storyRefs.length;i++){const match=String(storyRefs[i].image||'').match(/^data:(image\/(?:jpeg|png|webp));base64,(.+)$/i);if(!match)continue;const mime=match[1].toLowerCase(),bytes=Buffer.from(match[2],'base64'),extension=mime.includes('png')?'png':mime.includes('webp')?'webp':'jpg';form.append('image[]',new Blob([bytes],{type:mime}),`story-reference-${i+1}.${extension}`);}
