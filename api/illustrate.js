...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const castRefs=refs.map((r,i)=>({...r,attachmentIndex:i+1})).filter(r=>!['edit-source','continuity-artwork'].includes(String(r.kind||'')));
    const bookArtworkRefs=refs.map((r,i)=>({...r,attachmentIndex:i+1})).filter(r=>['edit-source','continuity-artwork'].includes(String(r.kind||'')));
    const hasReference = refs.length>0;
    const hasContinuityReference = Boolean(continuityImage) || bookArtworkRefs.some(r=>r.kind==='continuity-artwork');
    ...
    const identityDirection = castRefs.length
      ...
      : '';
    const wholeBookContinuityDirection = bookArtworkRefs.length
      ? `\nWHOLE-BOOK ARTWORK CONTINUITY — AUTHORITATIVE\nThe correction request includes artwork from across this SAME book. Attachment mapping: ${bookArtworkRefs.map(r=>`REFERENCE ${r.attachmentIndex} = ${r.name}`).join('; ')}. The EDIT MASTER is the current page and controls composition/staging unless the developer explicitly asks to change them. The other book artworks are continuity evidence. Review them together before editing. For every recurring creature, person, face, mouth and tooth pattern, horns, fur or hair, clothing, prop, building and other distinctive element, preserve the established identity seen consistently across the book. Do not redesign a recurring character merely because the current page is being corrected. If the current page has drifted from the established appearance in the other book images, restore the established appearance while making the requested correction. Do not copy another page's composition, pose or camera angle. Never duplicate a named character because that character appears in multiple reference images; references show the SAME recurring identity at different story moments.\n`
      : '';
