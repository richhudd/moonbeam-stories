...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // V251.02: developer-only whole-book continuity audit and canonical repair planning.
    if (['developer-whole-book-continuity','developer-investigate-continuity','developer-continuity-repair-plan'].includes(String(body.action || '').trim())) {
      ...
      const action=String(body.action||''),story=body.story||{},pages=Array.isArray(body.pages)?body.pages.slice(0,20):[];
      if(!pages.length)return res.status(400).json({error:'No book pages were supplied.'});
      const full=[story.opening,...(Array.isArray(story.pages)?story.pages.map(p=>p?.text||''):[]),story.closing].filter(Boolean).join('\n\n');
      let instruction;
      if(action==='developer-whole-book-continuity'||action==='developer-investigate-continuity'){
        const userIssue=String(body.userIssue||'').trim();
        const issueType=String(body.issueType||'').trim()==='illustration'?'illustration':(String(body.issueType||'').trim()==='text'?'text':'both');
        const scopeRule=issueType==='text'?'HUMAN-DIRECTED TEXT EDIT: investigate only the human editor’s requested text correction or improvement across the complete story. It may be continuity, clarity, pacing, tone, humour, age-appropriateness or another writing change. Do not inspect illustrations or invent unrelated edits.':issueType==='illustration'?'HUMAN-DIRECTED ILLUSTRATION EDIT: inspect the supplied finished illustrations only to locate and understand the human editor’s requested visual correction or improvement. It may be continuity or a small art-direction change. Use story text and page scene facts as authoritative context. Do not rewrite coherent prose to accommodate an image. Preserve unrelated visual details.':'Inspect both text and illustrations only as required by the human instruction.';
        instruction=`You are investigating a HUMAN-REQUESTED correction or improvement in a finished children's picture book. ${scopeRule}\n\nCOMPLETE STORY:\n${full}\n\nAudit BOTH dimensions below:
1. CROSS-PAGE CONTINUITY: find material conflicts where the same persistent fact changes incompatibly across the book: possessions and object location, object size/identity, clothing or equipment, vehicles, injuries/damage, locations, time-of-day facts, or other state that should persist until the story changes it. Example: a normal red flag is planted on one page, but later a tiny flag falls from the character's pocket.
2. WITHIN-PAGE TEXT/ILLUSTRATION STATE: for EVERY supplied illustration, compare it directly with that same page's text. Check action sequence, timing and consequences as well as nouns and appearance. All depicted elements must belong to a compatible moment. Example: if the text says Sam walked towards the rocket and repaired a fallen flag, an image showing Sam still some distance away walking towards the rocket while the flag is already repaired mixes a pre-action character state with a post-action object state and MUST be reported.

A coherent passage of text is not evidence that the illustrated page is coherent. If the prose makes sense but its illustration contradicts it, identify the ILLUSTRATION as the conflicting source; do not propose rewriting good prose merely to accommodate an erroneous image.\n\n${userIssue?`\n\nTHE HUMAN EDITOR HAS REQUESTED THIS SPECIFIC CHANGE OR INVESTIGATION (${issueType.toUpperCase()}):\n${userIssue}\nFollow this request only in the editor-selected ${issueType} scope. For a subjective improvement request, do not demand that it be a factual contradiction: identify the relevant page(s) and the smallest change that fulfils the instruction. Do not perform an unrelated automatic audit.`:''}\n\nDo NOT invent explanations to reconcile contradictory generations. Do NOT choose which version is correct. Instead expose the competing versions so the human developer can choose canon. Ignore harmless artistic variation, omissions, camera angle, stylisation and unimportant scale changes.\n\nThe supplied pageIndex is the actual reading-spread index: opening=0, then story pages in order, then closing. Preserve those exact indexes in your output.\n\nReturn JSON only: {"conflicts":[{"subject":"short name","summary":"brief neutral description of the conflict","evidence":[{"pageIndex":0,"source":"text|illustration","detail":"what this page establishes"}],"options":[{"label":"short human-readable choice","fact":"precise canonical fact that would govern the whole book"}]}]}. Give 2-4 genuinely supported canonical options per conflict; do not add fanciful compromises. If none, {"conflicts":[]}.`;
      }else{
        const conflict=body.conflict||{},canon=String(body.canon||'').trim();if(!canon)return res.status(400).json({error:'A canonical fact is required.'});
        instruction=`You are planning repairs to a finished children's picture book after the human developer has resolved a continuity conflict.\n\nCOMPLETE STORY:\n${full}\n\nCONFLICT:\n${JSON.stringify(conflict)}\n\nAUTHORITATIVE CANON CHOSEN BY HUMAN:\n${canon}\n\nIdentify every page that conflicts with this canon. Do not change unrelated material and do not invent explanations. For text conflicts, provide the COMPLETE minimally corrected replacement page text in suggestedText, preserving voice, tense, plot, approximate length and unrelated wording. For illustration repairs, identify only the affected page(s) and state the exact requested visual delta. Treat the EXISTING illustration as the visual master: preserve composition, framing, camera angle, characters, likenesses, poses, clothing, lighting, colours, background, objects, scale and style unless the human request specifically requires one of those details to change. A small request such as “Sam should look more frightened” means change only the relevant expression, not create a new scene. If both conflict, use kind "text+illustration" and include suggestedText. Return JSON only: {"repairs":[{"pageIndex":0,"kind":"text|illustration|text+illustration","action":"precise minimal repair instruction","suggestedText":"complete replacement page text when kind includes text, otherwise empty string"}]}.`;
      }
      const content=[{type:'input_text',text:instruction}];
      for(const p of pages){
        content.push({type:'input_text',text:`PAGE ${Number(p.pageIndex)} TEXT:\n${String(p.text||'')}`});
        const img=String(p.image||'');if(/^data:image\/(?:jpeg|png|webp);base64,/i.test(img))content.push({type:'input_image',image_url:img});
      }
      const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5.6-luna',input:[{role:'user',content}],max_output_tokens:6000})});
      ...
      text=text.trim().replace(/^```(?:json)?\s*/i,'').replace(/\s*```$/,'');let parsed;try{parsed=JSON.parse(text)}catch{throw new Error('The whole-book continuity editor returned invalid JSON.')}
      if(action==='developer-whole-book-continuity'||action==='developer-investigate-continuity')return res.status(200).json({conflicts:Array.isArray(parsed.conflicts)?parsed.conflicts:[]});
      return res.status(200).json({repairs:Array.isArray(parsed.repairs)?parsed.repairs:[]});
    }

    // V251.01: developer-only editorial check of page text against finished illustrations.
    if (String(body.action || '').trim() === 'developer-illustration-text-check') {
      const user = await verifyMoonbeamUser(req);
      const developerEmail = String(process.env.MOONBEAM_DEVELOPER_EMAIL || '').trim().toLowerCase();
      if (!developerEmail || String(user.email || '').trim().toLowerCase() !== developerEmail) return res.status(403).json({ error: 'Developer access only.' });
      const story=body.story||{},pages=Array.isArray(body.pages)?body.pages.slice(0,20):[];
      if(!pages.length)return res.status(400).json({error:'No illustrated pages were supplied.'});
      const full=[story.opening,...(Array.isArray(story.pages)?story.pages.map(p=>p?.text||''):[]),story.closing].filter(Boolean).join('\n\n');
      const content=[{type:'input_text',text:`You are the final continuity editor for a finished children's picture book. Compare each supplied finished illustration with its corresponding page text, using the COMPLETE STORY below to protect established plot facts.\n\nCOMPLETE STORY — HIGHEST AUTHORITY:\n${full}\n\nRULES:\n1. Report genuine physical, logical, factual OR action/timing contradictions between a page illustration and its text. Explicitly check whether people, objects and consequences all belong to a compatible moment in the sequence described. A picture must not mix a character before an action with an object already showing the result of that action. Ignore harmless artistic omissions, composition choices and incidental visual details.\n2. Established story facts outrank the illustration. If making the text agree with an image would contradict an established plot fact elsewhere, do NOT rewrite the plot around the erroneous image; omit that proposed text correction.\n3. When a mismatch can safely be fixed in prose, rewrite only that page's text, minimally. Preserve voice, tense, approximate length, plot and characterisation.\n4. Do not improve style, polish prose, or make changes unrelated to the inconsistency.\n5. Return JSON only, exactly: {"findings":[{"pageIndex":0,"reason":"brief explanation","suggestedText":"complete replacement text for that page"}]}. pageIndex is zero-based within the supplied story pages. If there are no genuine correctable inconsistencies, return {"findings":[]}.\n\nThe following images arrive in page order. Each image is preceded by a label containing its zero-based pageIndex and current text.`}];
      for(const p of pages){
        const m=String(p.image||'').match(/^data:(image\/(?:jpeg|png|webp));base64,(.+)$/i);if(!m)continue;
        content.push({type:'input_text',text:`PAGE ${Number(p.pageIndex)} CURRENT TEXT:\n${String(p.text||'')}`});
        content.push({type:'input_image',image_url:String(p.image)});
      }
      ...
      const raw=await r.text();let data={};try{data=JSON.parse(raw)}catch{}if(!r.ok){const e=data?.error;throw new Error(typeof e==='string'?e:(e?.message||`OpenAI returned HTTP ${r.status}`))}
      let text=typeof data.output_text==='string'?data.output_text:'';if(!text&&Array.isArray(data.output))for(const item of data.output)for(const part of(item.content||[]))if(typeof part.text==='string')text+=part.text;
      text=text.trim().replace(/^```(?:json)?\s*/i,'').replace(/\s*```$/,'');let parsed;try{parsed=JSON.parse(text)}catch{throw new Error('The continuity checker returned invalid JSON.')}
      const findings=Array.isArray(parsed.findings)?parsed.findings.filter(f=>Number.isInteger(f.pageIndex)&&typeof f.suggestedText==='string'&&f.suggestedText.trim()).map(f=>({pageIndex:f.pageIndex,reason:String(f.reason||'').trim(),currentText:String(pages.find(p=>Number(p.pageIndex)===f.pageIndex)?.text||''),suggestedText:f.suggestedText.trim()})):[];
      return res.status(200).json({findings});
    }

    // V251.00: developer-only correction of stored cover title / author-dedication text.
    if (String(body.action || '').trim() === 'developer-cover-text') {
      const user = await verifyMoonbeamUser(req);
      const developerEmail = String(process.env.MOONBEAM_DEVELOPER_EMAIL || '').trim().toLowerCase();
      if (!developerEmail || String(user.email || '').trim().toLowerCase() !== developerEmail) return res.status(403).json({ error: 'Developer access only.' });
      const kind=String(body.kind||'').trim(),instruction=String(body.instruction||'').trim();
      if(!['title','byline'].includes(kind)||!instruction)return res.status(400).json({error:'A cover field and correction instruction are required.'});
      const current=kind==='title'?String(body.currentTitle||''):String(body.currentByline||'');
      const label=kind==='title'?'book title':'cover author/dedication line';
      const prompt=`You are applying one developer-authorized correction to a finished children's book cover. Current ${label}: ${current}\nDeveloper instruction: ${instruction}\nReturn ONLY the final replacement ${label}, with no quotation marks, explanation, labels or commentary. Follow the developer instruction exactly. Do not rewrite or embellish wording they have explicitly supplied.`;
      const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5.6-luna',input:prompt,max_output_tokens:120})});
      const raw=await r.text();let data={};try{data=JSON.parse(raw)}catch{}if(!r.ok){const e=data?.error;throw new Error(typeof e==='string'?e:(e?.message||`OpenAI returned HTTP ${r.status}`))}
      let value=typeof data.output_text==='string'?data.output_text:'';if(!value&&Array.isArray(data.output))for(const item of data.output)for(const part of(item.content||[]))if(typeof part.text==='string')value+=part.text;
      value=value.trim().replace(/^["“”']|["“”']$/g,'').trim();if(!value)throw new Error('The corrected cover text came back empty.');
...
Write with genuine imaginative freedom. Find a compelling central idea that makes this particular story worth telling, and develop it with confidence.
Do not assemble the story from compulsory children's-story ingredients. There is no required magical companion, special object, quest, mystery, hidden door, twist, lesson or fantasy device. Equally, do not avoid any of those things when they arise naturally from the story. Prefer one coherent conception over a collection of generic ingredients.
