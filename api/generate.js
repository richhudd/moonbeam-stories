...
CAST MODEL SHEET: For EACH recurring non-photo character specify: name/role; exact age when human (never an age range); sex where relevant; apparent height/build relative to the child heroes; skin tone or fur/material colour; eye colour; face shape/distinctive facial features; exact hair/fur colour, length, texture and hairstyle; established clothing colours/items; and any permanent distinctive feature/accessory. For recurring animals, robots or fantastical beings give equally concrete fixed species/body/material/colour/size/features. For every dog, use the supplied breed when present to infer realistic adult/juvenile body proportions and RELATIVE SIZE beside the human characters; a Chihuahua must remain tiny, a Jack Russell small, a Labrador medium-large, an Irish Wolfhound very large/tall, etc. If breed is absent, infer approximate size from any supplied reference photo when possible; otherwise use a plausible medium size. Never arbitrarily rescale a dog between scenes. Do not leave recurring companions as vague phrases such as "a girl of similar age". Once defined, these details are immutable for the entire book unless the STORY itself explicitly requires a change.

WORLD MODEL SHEET: For EVERY plot-important recurring location, building, room, object, vehicle or machine, give it one stable name and specify its fixed visual identity: exact count; overall shape and proportions; materials and colours; weathering/condition; spatial layout; door/window/opening placement; and the number, design and placement of distinctive parts such as wheels, handles, controls, pedals, screens, rails, lenses or markings. These details are immutable unless a story event explicitly changes them. One named lighthouse must remain one lighthouse, never become two lighthouses, and retain the same architecture from every viewpoint. One wheel, crank, handle or control must remain the same size, construction and position instead of being redesigned on each page. Camera-angle changes must never create duplicates or alter the design. Every illustration_prompt must reuse the exact established names and must not redefine them inconsistently.

For ANY selected Cast member with a supplied reference photo — child, adult or pet — underlying physical identity comes authoritatively from that exact photo. Preserve the recognisable face, apparent age, hair, approximate skin tone, body proportions and other identifying physical characteristics shown by the reference; for pets preserve the recognisable species/breed appearance and proportions. Story-world clothing, costume, role, status, abilities and story-required fictional characteristics or transformations are free to follow the story and must not be mistaken for conflicting real-world identity. The bible should identify photographed Cast members by their supplied name, kind and narrative role and record any story-world appearance or continuity details needed for the book while keeping the underlying person or pet recognisable. A photographed adult is just as identity-locked as a photographed child, and a photographed pet is just as identity-locked as a photographed human. Every illustration_prompt must use the SAME supplied Cast names and preserve established continuity unless the STORY itself explicitly requires a change. Illustration prompts describe scene action/content only; they must not specify or vary the rendering/art style. Do not include text or lettering in illustrations.`;
