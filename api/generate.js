...
- The selected plot engine is the central narrative mechanism. Do not turn every story into a mystery, secret discovery or quest.`;
    ...
    const roleRules = `HERO: identifies whose story this fundamentally is. If there are multiple heroes, treat them as genuine co-heroes, but do not mechanically divide actions, dialogue or page time equally; let the particular story determine the most natural focus from moment to moment.
SUPPORTING_CHILD / SUPPORTING_ADULT / COMPANION: these are selected familiar Cast members AVAILABLE to the author for this story, not characters who must accompany, help, supervise or appear continuously. Their role may be large, small, intermittent or scene-specific according to what this particular plot naturally needs. Do not invent errands, dialogue, reactions, assistance or token actions merely to prove that a selected Cast member is participating. A character may be physically present without being mentioned in the prose, and may be absent for substantial parts of the story.
An adult Cast relationship, when supplied, is optional identity/context information only. It is NOT an instruction that the adult must supervise, guide, reassure, rescue or support the heroes. Adults may sometimes do those things when the story naturally calls for them, but their behaviour must arise from the plot rather than from the SUPPORTING label.
...
CAST FLOW — HARD RULE: selected Cast is a palette, not a checklist. Never rotate mechanically through names (X does this, Y does that, Z does the other) or crowd scenes merely to include everyone. Use exactly the characters that the story beat naturally requires. A scene may contain one person, several, the whole Cast, or no named Cast if that is genuinely right for the story. Large ensemble scenes are welcome when the plot actually calls for them.
...
{"title":"string","opening":"string","opening_cast":["name"],"character_bible":"string","pages":[...exactly ${pageCount} page objects...],"closing":"string","closing_cast":["name"]}
...
Each pages array item MUST have exactly this shape: {"text":"string","illustration_prompt":"string","scene_cast":["name"]}.

SCENE CAST — MANDATORY
For opening_cast, every page scene_cast, and closing_cast, list ONLY the selected named Cast members who are physically visible in that particular illustration. Use their supplied names exactly. An empty array is allowed when no selected Cast member is visible. Do not include someone merely because they are selected for the story, mentioned in previous context, or present elsewhere in the book. Let scene membership flow naturally from the plot. The illustration_prompt must depict only the named Cast members actually present in that scene and must not squeeze absent Cast into the composition.
...
For ANY selected Cast member with a supplied reference photo — child, adult or pet — identity comes authoritatively from that exact photo. Do not invent an alternative appearance, face, age, ethnicity, hair, body type, species/breed appearance or other conflicting physical characteristics for that Cast member. The bible should identify photographed Cast members by name, kind and role and may record only story clothing and continuity details that do not conflict with the reference. A photographed adult is just as identity-locked as a photographed child, and a photographed pet is just as identity-locked as a photographed human. Every illustration_prompt must use the SAME character names and must not redefine, age, recolour, restyle or change the clothing of recurring characters. Illustration prompts describe scene action/content only; they must not specify or vary the rendering/art style. Do not include text or lettering in illustrations.`;
    ...
    function normaliseStory(story) {
      ...
      const validCastNames = cast.length ? cast.map(m=>String(m.name||'').trim()).filter(Boolean) : [String(child.name||'').trim()].filter(Boolean);
      const cleanSceneCast = (value, fallbackText='') => {
        const supplied = Array.isArray(value) ? value.map(x=>String(x||'').trim()).filter(Boolean) : null;
        const candidates = supplied === null ? validCastNames.filter(name=>new RegExp(`(^|[^\\p{L}\\p{N}])${name.replace(/[.*+?^${}()|[\\]\\\]/g,'\\$&')}([^\\p{L}\\p{N}]|$)`,'iu').test(String(fallbackText||''))) : supplied;
        const wanted = new Set(candidates.map(x=>x.toLocaleLowerCase()));
        return validCastNames.filter(name=>wanted.has(name.toLocaleLowerCase()));
      };
      const pages = Array.isArray(story.pages) ? story.pages.map(p => ({
        ...
        scene_cast: cleanSceneCast(p?.scene_cast, `${p?.text||''} ${p?.illustration_prompt||''}`)
      })).filter(p => p.text) : [];
      const normal = {
        title: typeof story.title === 'string' ? story.title.trim() : '',
        opening: typeof story.opening === 'string' ? story.opening.trim() : '',
        opening_cast: cleanSceneCast(story.opening_cast, story.opening||''),
        ...
        closing_cast: cleanSceneCast(story.closing_cast, story.closing||'')
      };
